...
class content_model extends model {
	...
	public function add_content($data,$isimport = 0) {
		...
		/*强制插入自定义分类*/
		//p(remote_filesize('t7.dbeltroad.org/uploadfile/2021/0916/20210916085913838.docx'));exit;
		if ($data['file'] && in_array($data['catid'],array(44,45))) {  
			//clearstatcache() 
			$inputinfo['system']['filesize']=sizecount(remote_filesize($_SERVER['SERVER_NAME'].$data['file']));
		}
		//p($inputinfo);exit;
		/*强制插入自定义分类end*/
	...
	public function status($ids = array(), $status = 99) {
		$this->content_check_db = bc_base::load_model('content_check_model');
		$this->message_db = bc_base::load_model('message_model');
	...
	private function update_category_items($catid,$action = 'add',$cache = 0) {
		$this->category_db = bc_base::load_model('category_model');
		if($action=='add') {
