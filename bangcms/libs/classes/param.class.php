...
class param {
	...
	public function __construct() {
		...
		$this->route_config = bc_base::load_config('route', SITE_URL) ? bc_base::load_config('route', SITE_URL) : bc_base::load_config('route', 'default');
	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		$var = bc_base::load_config('system','cookie_pre').$var;
		$_COOKIE[$var] = $value;
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($var.'['.$k.']', sys_auth($v, 'ENCODE'), $time, bc_base::load_config('system','cookie_path'), bc_base::load_config('system','cookie_domain'), $s);
			}
		} else {
			setcookie($var, sys_auth($value, 'ENCODE'), $time, bc_base::load_config('system','cookie_path'), bc_base::load_config('system','cookie_domain'), $s);
		}
	}
	...
	private function safe_deal($str) {
		$str = safe_replace(remove_xss($str));
