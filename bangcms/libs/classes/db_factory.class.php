...
final class db_factory {
	...
	public static function get_instance($db_config = '') {
		if($db_config == '') {
			$db_config = bc_base::load_config('database');
		}
	...
	public function connect($db_name) {
		...
		switch($this->db_config[$db_name]['type']) {
			case 'mysql' :
				bc_base::load_sys_class('mysql', '', 0);
				$object = new mysql();
				break;
			case 'mysqli' :
				//$object = bc_base::load_sys_class('mysqli');
				bc_base::load_sys_class('db_mysqli','',0);	
				$object = new db_mysqli();
				break;
			case 'access' :
				$object = bc_base::load_sys_class('db_access');
				break;
			case 'mssql' :
				$object = bc_base::load_sys_class('db_mssql');
				break;
			default :
				bc_base::load_sys_class('mysql', '', 0);
				$object = new mysql();
