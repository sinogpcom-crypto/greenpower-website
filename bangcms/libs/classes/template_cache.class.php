...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default') {
		...
		$tplfile = $_tpl = BC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
			...
			$module = str_replace('/', DIRECTORY_SEPARATOR, $module);
			$tplfile = $_tpl = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html';
		}
		if ($style != 'default' && !file_exists ( $tplfile )) {
			$style = 'default';
			$tplfile = BC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
		}
			...
			switch ($op) {
				...
				case 'get':
						...
						if (isset($page)) {
							...
							$str .= '$r = $get_db->sql_query("'.$sql.'");$s = $get_db->fetch_next();$pages=xwjpages($s[\'count\'], $page, $pagesize, $urlrule);';
							$str .= '$r = $get_db->sql_query("'.$sql.'");$s = $get_db->fetch_next();$pages2=pages2($s[\'count\'], $page, $pagesize, $urlrule);';
						}
				...
				case 'block':
					$str .= '$block_tag = bc_base::load_app_class(\'block_tag\', \'block\');';
					$str .= 'echo $block_tag->pc_tag('.self::arr_to_html($datas).');';
					break;
			}
		} else {
			if (!isset($action) || empty($action)) return false;
			if (module_exists($op) && file_exists(BC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.php')) {
				$str .= '$'.$op.'_tag = bc_base::load_app_class("'.$op.'_tag", "'.$op.'");if (method_exists($'.$op.'_tag, \''.$action.'\')) {';	
				if (isset($start) && intval($start)) {
				...
				if (isset($page)) {
					...
					$str .= '$pages = xwjpages($'.$op.'_total, $page, $pagesize, $urlrule);';
					$str .= '$pages2 = pages2($'.$op.'_total, $page, $pagesize, $urlrule);';
				}
				$str .= 'global $'.$return.'; $'.$return.' = $'.$op.'_tag->'.$action.'('.self::arr_to_html($datas).');';
				$str .= '}';
			} 
		}
		if (!empty($cache) && !isset($page)) {
