...
class attachment {
	...
	function __construct($module='', $catid = 0,$siteid = 0,$upload_dir = '') {
		...
		$this->upload_root = bc_base::load_config('system','upload_path');
	...
	function add($uploadedfile) {
		...
		$uploadedfile['status'] = bc_base::load_config('system','attachment_stat') ? 0 : 1;
	...
	*/
	...
	function uploaderror($id = 0)	{
		file_put_contents(BANGCMS_PATH.'xxx.txt', $id);
	}
