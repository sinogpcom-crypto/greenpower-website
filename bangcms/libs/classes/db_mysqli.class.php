...
final class db_mysqli {
	...
	public function select($data, $table, $where = '', $limit = '', $order = '', $group = '', $key = '') {
       ...
       // if($limit && stripos($limit, ',') === false) $limit = $limit.',10';
		...
		//$datalist = recursive_array($datalist,'htmlspecialchars');
		$this->free_result();
		return $datalist;
	}
	...
	public function fetch_next($type=MYSQLI_ASSOC) {
	   //phpcms元框架的问题 mysqli只能用mysqli_fetch_array xieyu
	...
	public function halt($message = '', $sql = '') {
	    ...
	    //db_error_send_mail($this->errormsg."<br/>".IP."<br/>".date("Y-m-d H:i:s",SYS_TIME)."<br/>".ROUTE_M."-".ROUTE_C."-".ROUTE_A."<br/>".APP_FULLNAME."<br/>".$_SERVER['HTTP_USER_AGENT']);
	    
	    my_error_handler($this->errno(),$this->errormsg, ROUTE_M.'_'.ROUTE_C.'_'.ROUTE_A, $errline);//2014年4月28日11:45:39 记录出错日志
	    if($this->config['debug']) {
	        ...
	        //output_to_app(false,'数据异常，稍后再试' ,array()) ;
	        return false;
	    }
	}
	...
	public function count_sql($sql)
	{
		...
		$this->query( 'select count(*) AS num from '.$sql);
		$res = $this->fetch_next();
		$this->free_result();
		return $res['num'];
	}
	 ...
	 * 执行sql语句
	 * @param $data         需要查询的字段值[例`name`,`gender`,`birthday`]
	 * @param $sql                 sql语句[不包括select和from]
	 * @param $order         排序
	 * @param $limit         查询限制
	 * @return array                查询结果集数组
	 */
