 ...
 * $Date: 2015-02-28 16:12:35 +0800 (六, 28 2月 2015) $
 * $Id: global.func.php 250 2015-02-28 08:12:35Z shidongdong $
 *
 * @copyright			(C) 2009-2014 BangCMS
 * @license				http://builder.netbang.com.cn/license/
 * @package libs\functions\global.func.php
 */
...
function sys_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0) {
	$key_length = 4;
	$key = md5($key != '' ? $key : bc_base::load_config('system', 'auth_key'));
	$fixedkey = md5($key);
...
function L($language = 'no_language',$pars = array(), $modules = '') {
	...
	if(defined('IN_ADMIN')) {
		...
		$lang = bc_base::load_config('system','lang');
	}
	if(!$LANG) {
		require_once BC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system.lang.php';
		if(defined('IN_ADMIN')) require_once BC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system_menu.lang.php';
		if(file_exists(BC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php')) require_once BC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php';
	}
	if(!empty($modules)) {
		$modules = explode(',',$modules);
		foreach($modules AS $m) {
			if(!isset($LANG_MODULES[$m])) require_once BC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.$m.'.lang.php';
		}
	}
...
function template($module = 'content', $template = 'index', $style = '') {
	...
	$template_cache = bc_base::load_sys_class('template_cache');
	//编译后的文件：Project/caches/caches_template/default/模块/*.php
	$compiledtplfile = BANGCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
	//模板html文件：Project/bangcms/templates/default/模块/*.html
	$template_html_file = BC_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
	if(file_exists($template_html_file)) { //模板存在：生成指定模板缓存
		if(!file_exists($compiledtplfile) || (@filemtime($template_html_file) > @filemtime($compiledtplfile))) {
		    //缓存不存在或模板修改过：编译模板
			$template_cache->template_compile($module, $template, $style);
		}
	} else { //模板不存在
		$compiledtplfile = BANGCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.php';
		$template_html_file = BC_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
		if(!file_exists($compiledtplfile) || (file_exists($template_html_file) && filemtime($template_html_file) > filemtime($compiledtplfile))) {
		    //缓存不存在或默认模板存在且模板修改过：编译默认模板
			$template_cache->template_compile($module, $template, 'default');
		} elseif (!file_exists($template_html_file)) {
		    //默认模板不存在：报错处理
			showmessage('Template does not exist.'.$template_html_file);
		}
	}
...
/**
 * 输出自定义错误，根据系统配置load_config('system','errorlog')中的选项进行错误处理
 *
 * @param $errno 错误号
 * @param $errstr 错误描述
 ...
 * showmessage('登录成功', array('默认跳转地址'=>'http://www.zhengbang.com.cn'));
 ...
 * @param $config 配置名称
 * @param $timeout 过期时间
 * @return  mixed				缓存文件大小/false
 */
function setcache($name, $data, $filepath='', $type='file', $config='', $timeout=0) {
	bc_base::load_sys_class('cache_factory','',0);
	if($config) {
		$cacheconfig = bc_base::load_config('cache');
...
function getcacheinfo($name, $filepath='', $type='file', $config='') {
	bc_base::load_sys_class('cache_factory');
	if($config) {
		$cacheconfig = bc_base::load_config('cache');
		$cache = cache_factory::get_instance($cacheconfig)->get_cache($config);
	} else {
...
//		$multipage .= '<a class="a1">'.$num.L('page_item').'</a>';		//共 xx 条
		if($curr_page>0) {
			$multipage .= ' <a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="a1">'.L('previous').'</a>';
			if($curr_page==1) {
				$multipage .= ' <a href="javascript:;" class="on">1</a>';
		...
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				...
				$multipage .= ' <a href="javascript:;" class="on">'.$i.'</a>';
			}
		}
		if($curr_page<$pages) {
			...
			$multipage .= ' <a href="javascript:;" class="on">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
		}
	}
 ...
 * @return string 分页html代码
 */
function xwjpages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		//$multipage .= '<li><a>'.$num.L('page_item').'</a></li>';
		if($curr_page>0) {
			$multipage .= ' <span class="page-prev"><a href="'.pageurl($urlrule, $curr_page-1, $array).'"><</a></span><ul class="clearfix">';
			if($curr_page==1) {
				$multipage .= '<li class="pagenumact">1</li>';
			} elseif($curr_page>6 && $more) {
		...
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				...
				$multipage .= '<li class="pagenumact">'.$i.'</li>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5) {
				$multipage .= ' <li>..</li><li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> </ul><span class="page-next"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">></a></span>';
			} else {
				$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> </ul><span class="page-next"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">></a></span>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= '<li class="pagenumact">'.$pages.'</li> </ul><span class="page-next"><a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">></a></span>';
		} else {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> </ul><span class="page-next"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">></a></span>';
		}
	}
...
function ztpages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			$multipage .= ' <a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="d_last fl">'.L('previous').'</a>';
			if($curr_page==1) {
				$multipage .= ' <span class="d_span fl">1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <span class="d_span fl"><a href="'.pageurl($urlrule, 1, $array).'">1</a></span><span class="d_span fl">..</span>';
			} else {
				$multipage .= '<span class="d_span fl"> <a href="'.pageurl($urlrule, 1, $array).'">1</a></span>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <span class="d_span fl"><a href="'.pageurl($urlrule, $i, $array).'">'.$i.'</a></span>';
			} else {
				$multipage .= ' <span class="d_span fl">'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= '<span class="d_span fl"> ..</span><span class="d_span fl"><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></span><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="d_next fl">'.L('next').'</a>';
			} else {
				$multipage .= ' <span class="d_span fl"><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> </span><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="d_next fl">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span class="d_span fl">'.$pages.'</span> <a href="'.pageurl($urlrule, $curr_page, $array).'" class="d_next fl">'.L('next').'</a>';
		} else {
			$multipage .= '<span class="d_span fl"> <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></span> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="d_next fl">'.L('next').'</a>';
		}
	}
...
function my_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		$multipage .= '<ul class="clearfix">';
		if($curr_page>0) {
			$multipage .= '<li class="l-01 fl"><a href="'.pageurl($urlrule, $curr_page-1, $array).'" ><img src="'.IMG_PATH.'img/page-l.png"></a></li>';
			if($curr_page==1) {
				$multipage .= '<li class="on l-02 fl"><a href="'.pageurl($urlrule, 1, $array).'">1</a></li>';
			} elseif($curr_page>6 && $more) {
				$multipage .= '<li class="l-02 l-03none fl"> <a href="'.pageurl($urlrule, 1, $array).'">1</a>..</li>';
			} else {
				$multipage .= ' <li class="l-02 fl"><a href="'.pageurl($urlrule, 1, $array).'">1</a></li>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <li class="l-02 fl"><a href="'.pageurl($urlrule, $i, $array).'">'.$i.'</a></li>';
			} else {
				$multipage .= '<li class="on l-02 fl"><a href="'.pageurl($urlrule, $i, $array).'">'.$i.'</a></li>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' <li class="l-02 fl">..<a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a><li> <li class="l-02 fl"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
			} else {
				$multipage .= ' <li class="l-02 fl"><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li class="l-02 fl"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <li class="on l-02 fl"><a href="'.pageurl($urlrule, $pages, $array).'" >'.$pages.'</a></li> <li class="l-02 fl"><a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a></li>';
		} else {
			$multipage .= ' <li class="l-02 fl"><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> </li> <li class="l-02 fl"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
		}
		$multipage .='</ul>';
...
function show_ad($siteid, $id) {
	...
	$p = bc_base::load_model('poster_model');
 ...
 * @param $is_userid $uid是否为userid，如果为真，执行sql查询此用户的phpssouid
 * @param $size 头像大小 有四种[30x30 45x45 90x90 180x180] 默认30
 * @return false|string 头像URL
 */
function get_memberavatar($uid, $is_userid='', $size='30') {
    $sso_open = bc_base::load_config('system', 'phpsso'); //单点登录是否开启
	if($is_userid && $sso_open) {
        $db = bc_base::load_model('member_model');
    ...
    if ($sso_open){ //单点登录开启
        bc_base::load_app_class('client', 'member', 0);
        define('APPID', bc_base::load_config('system', 'phpsso_appid'));
        $phpsso_api_url = bc_base::load_config('system', 'phpsso_api_url');
        $phpsso_auth_key = bc_base::load_config('system', 'phpsso_auth_key');
        $client = new client($phpsso_api_url, $phpsso_auth_key);
        $avatar = $client->ps_getavatar($uid);
    }else { //单点登录关闭
        bc_base::load_app_class('client_member', 'member', 0);
        $client = new client_member();
        $avatar = $client->get_avatar($uid);
    }
...
function thumb($imgurl, $width = 100, $height = 100 ,$autocut = 1, $smallpic = 'nopic.gif') {
	global $image;
	$upload_url = bc_base::load_config('system','upload_url');
	$upload_path = bc_base::load_config('system','upload_path');
	...
	if(!is_object($image)) {
		bc_base::load_sys_class('image','','0');
		$image = new image(1,0);
	}
 ...
 * @param $relation_model_id  关联模块（1:n）的关联模型id
 * @return string URL
 */
function go($catid,$id, $allurl = 0, $relation_model_id = 0) {
	static $category;
	if(empty($category)) {
		$siteids = getcache('category_content','commons');
		$siteid = $siteids[$catid];
	...
	if ($relation_model_id) {
		$db = bc_base::load_model('detail_model');
		$db->set_model($relation_model_id);
	} else {
...
function upload_key($args) {
	$pc_auth_key = md5(bc_base::load_config('system','auth_key').$_SERVER['HTTP_USER_AGENT']);
 ...
 * @param string $font 字体名称 字体请放于bangcms\libs\data\font下
 ...
 * @return string
 */
function get_bc_version($type='') {
	$version = bc_base::load_config('version');
	if($type==1) {
		return $version['bc_version'];
	} elseif($type==2) {
		return $version['bc_release'];
	} else {
		return $version['bc_version'].' '.$version['bc_release'];
	}
}
/**
 * 运行钩子（插件使用）
...
function runhook($method) {
	...
	if(!empty($hook_appid)) {
		foreach($hook_appid as $appid => $p) {
			$pluginfilepath = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$p.DIRECTORY_SEPARATOR.'hook.class.php';
			$getpclass = TRUE;
...
function p_template($plugin = 'content', $template = 'index',$style='default') {
	if(!$style) $style = 'default';
	$template_cache = bc_base::load_sys_class('template_cache');
	$compiledtplfile = BANGCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.$template.'.php';
    $template_html_file = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html';
	if(!file_exists($compiledtplfile) || (file_exists($template_html_file) && filemtime($template_html_file) > filemtime($compiledtplfile))) {
...
function get_vid($contentid = 0, $catid = 0, $isspecial = 0) {
	...
	if (!$isspecial) {
		...
		$video_content = bc_base::load_model('video_content_model');
		$r = $video_content->get_one(array('contentid'=>$contentid, 'modelid'=>$modelid), 'videoid', '`listorder` ASC');
		$video_store =bc_base::load_model('video_store_model');
		return $video_store->get_one(array('videoid'=>$r['videoid']));
	} else {
		$special_content = bc_base::load_model('special_content_model');
		$contentid = intval($contentid);
		$video_store =bc_base::load_model('video_store_model');
 ...
 */
 ...
 /**
  * 获取downfiles类型字段的文件地址
  *
  * @param	string	$data	字符串
  * @return	string	返回字符串，如果，data为空，则返回空字符串
  */
 function get_downfiles_href($data) {
 	if (!$data) return '';
 	preg_match("/href=\'(.*)\'/iUs", $data, $match);
 	return $match['1'];
 }
 /**
  * 获取扩展配置数据
  ...
  */
 function get_extend_setting($key){
     if (empty($key)) return false;
     $data = array();
     $model = bc_base::load_model('extend_setting_model');
     $data = $model->get($key);
     return $data;
 }
 /**
  ...
  * @param mixed $data 数组或其他类型的配置数据
  * @return boolean
  */
 function set_extend_setting($key,$data){
     if (empty($key) || empty($data)) return false;
     $model = bc_base::load_model('extend_setting_model');
     return $model->set($key,$data);
 }
 /**
  * 删除扩展配置数据
  * @param string $key
  ...
  */
 function clear_extend_setting($key){
     if (empty($key)) return false;
     $model = bc_base::load_model('extend_setting_model');
     return $model->clear($key);
 }
 /**
  * 获取UCenter数据库配置
  */
 function get_uc_database() {
     $config = bc_base::load_config('system');
 ...
 function get_default_show(){
 	return array('title', 'updatetime', 'listorder');
 }
 /**
  * 获取后台默认筛选的字段名
  */
 function get_default_listfilter_show(){
 	return array('title','description','username');
 }
 function getCity($ip = '')
{
    if($ip == ''){
