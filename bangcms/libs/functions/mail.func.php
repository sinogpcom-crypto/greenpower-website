 ...
 * $Id: mail.func.php 152 2014-11-24 01:14:56Z jiaokun $
 * @copyright			(C) 2009-2014 BangCMS
 * @license				http://builder.netbang.com.cn/license/
 * @package libs\functions\mail.func.php
 */
/**
 * 邮件日志：目前空实现
 * @param string $mode
...
function sendmail($toemail, $subject, $message, $from='',$cfg = array(), $sitename='') {
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: BANGCMS {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=".CHARSET."{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." bangcms\r\n");
	$lastmessage = fgets($fp, 512);
