...
class payment extends admin {
	...
	function __construct() {
		...
		parent::__construct();
		$this->db = bc_base::load_model('pay_payment_model');
		$this->account_db = bc_base::load_model('pay_account_model');
		$this->member_db = bc_base::load_model('member_model');
		$this->modules_path = BC_PATH.'modules'.DIRECTORY_SEPARATOR.'pay';
		bc_base::load_app_class('pay_method','','0');
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($userinfo) {	
				...
				if($_POST['pay_unit']) {
					...
					$receipts = bc_base::load_app_class('receipts');
					...
					$spend = bc_base::load_app_class('spend');
					$func = $_POST['pay_type'] == '1' ? 'amount' :'point';
