...
class file extends admin {
	...
	public function __construct() {
		...
		$this->filepath = BC_PATH.'templates'.DIRECTORY_SEPARATOR.$this->style.DIRECTORY_SEPARATOR;
		...
		$this->tpl_edit = bc_base::load_config('system', 'tpl_edit');
		parent::__construct();
	}
	...
	public function init() {
		...
		$local = str_replace(array(BC_PATH, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('',DIRECTORY_SEPARATOR), $filepath);
		if (substr($local, -1, 1) == '.') {
	...
	public function public_ajax_get() {
		$op_tag = bc_base::load_app_class($_GET['op']."_tag", $_GET['op']);
	...
	public function edit_pc_tag() {
		...
		switch ($op) {
			...
			case 'get':
				...
				$dbsource_db = bc_base::load_model('dbsource_model');
			...
			default:
				if (!file_exists(BC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.php')) {
					showmessage(L('the_module_will_not_support_the_operation'));
				}
				$op_tag = bc_base::load_app_class($op."_tag", $op);
