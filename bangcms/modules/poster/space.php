...
class space extends admin {
	...
	function __construct() {
		...
		$this->db = bc_base::load_model('poster_space_model');
	}
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if (isset($_POST['old_type']) && $_POST['old_type']!=$space['type']) {
				$poster_db = bc_base::load_model('poster_model');
	...
	public function public_preview() {
		if (is_numeric($_GET['spaceid'])) {
			...
			if ($r['type']=='code') {
				$db = bc_base::load_model('poster_model');
	...
	private function template_type() {
		bc_base::load_app_func('global','poster');
		return get_types();
	}
	...
	public function poster_template() {
		...
		$templatedir = BC_PATH.$tpl_root.bc_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.'poster'.DIRECTORY_SEPARATOR;
