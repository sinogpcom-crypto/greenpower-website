...
class index {
	...
	public function init() {
		...
		$siteid =& $this->siteid;
		$username = param::get_cookie('_username',L('bangcms_friends'));
		$userid = param::get_cookie('_userid');
		
		$comment_setting_db = bc_base::load_model('comment_setting_model');
	...
	public function post() {
		$comment = bc_base::load_app_class('comment');
		$id = isset($_GET['id']) && intval($_GET['id']) ? intval($_GET['id']) : '';
		$SITE = siteinfo($this->siteid);
		$username = param::get_cookie('_username',$SITE['name'].L('bangcms_friends'));
		$userid = param::get_cookie('_userid');
		$comment_setting_db = bc_base::load_model('comment_setting_model');
		$setting = $comment_setting_db->get_one(array('siteid'=>$this->siteid));
		if (!empty($setting)) {
	...
	public function ajax() {
		...
		$pc_tag = bc_base::load_app_class('comment_tag');
		$comment = array();
	...
	protected function _show_msg($msg, $url = '', $status = 0) {
		...
		switch ($this->format) {
			case 'json':
				$msg = bc_base::load_config('system', 'charset') == 'gbk' ? iconv('gbk', 'utf-8', $msg) : $msg;
