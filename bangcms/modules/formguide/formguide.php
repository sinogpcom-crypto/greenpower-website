...
class formguide extends admin {
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			...
			define('MODEL_PATH',BC_PATH.'modules'.DIRECTORY_SEPARATOR.'formguide'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
			...
			bc_base::load_sys_class('form', '', false);
	...
	public function delete() {
		...
		if (isset($_GET['formid']) && !empty($_GET['formid'])) {
			$formid = intval($_GET['formid']);
			$m_db = bc_base::load_model('sitemodel_field_model');
	...
	private function sql_split($sql) {
		...
		$sql = str_replace("bangcms_form_table", $this->m_db->db_tablepre.'form_'.$this->tablename, $sql);
		$ret = array();
		$num = 0;
