...
class index extends foreground {
	function __construct() {
		...
		$this->message_group_db = bc_base::load_model('message_group_model');
		$this->message_data_db = bc_base::load_model('message_data_model');
	...
	public function public_name() {
		$username = isset($_GET['username']) && trim($_GET['username']) ? (bc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['username'])) : trim($_GET['username'])) : exit('0');
