...
class message extends admin {
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('message_model');
		$this->group_db = bc_base::load_model('message_group_model');
	 ...
	 */
	 public function public_name() {
		$tousername = isset($_GET['tousername']) && trim($_GET['tousername']) ? (bc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['tousername'])) : trim($_GET['tousername'])) : exit('0');
	 	//不能发给自己
		if($tousername == $this->_username){
				exit('0');
			}
		//判断用户名是否存在
		$member_interface = bc_base::load_app_class('member_interface', 'member');
		if ($tousername) {
	...
	public function message_send() {
		if(isset($_POST['dosubmit'])) {
			...
			//LOAD 会员组模型
			$member_group = bc_base::load_model('member_group_model');
			$member_group_infos = $member_group->select('','*','',$order = 'groupid ASC');
			//LOAD 管理员角色模型
			$role = bc_base::load_model('admin_role_model');
