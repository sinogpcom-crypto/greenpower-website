...
class manage extends admin {
	...
	function __construct() {
		...
		$this->db= bc_base::load_model('attachment_model');
		$this->attachment = bc_base::load_sys_class('attachment');
	...
	public function pulic_dirmode_del() {
		...
		$tmpdir = BANGCMS_PATH.$tmpdirs[0].'/';
		if($tmpdir!=$this->upload_path) {
			showmessage(L('illegal_operation'));
		}
		$file = BANGCMS_PATH.$dir.DIRECTORY_SEPARATOR.$filename;
	...
	public function delete() {
		$aid = $_GET['aid'];
		$attachment_index = bc_base::load_model('attachment_index_model');
