...
class attachments {
	...
	function __construct() {
		...
		$this->upload_url = bc_base::load_config('system','upload_url');
		$this->upload_path = bc_base::load_config('system','upload_path');		
	...
	public function upload() {
		...
		bc_base::load_sys_class('attachment','',0);
	...
	public function swfupload(){
		...
		if(isset($_POST['dosubmit'])){
			if( $_POST['swf_auth_key'] != md5(bc_base::load_config('system','auth_key').$_POST['SWFUPLOADSESSID']) || ($_POST['isadmin']==0 && !$grouplist[$_POST['groupid']]['allowattachment'])) exit();
			bc_base::load_sys_class('attachment','',0);
	...
	public function crop_upload() {
		if (isset($GLOBALS["HTTP_RAW_POST_DATA"])) {
			...
			if (isset($_GET['file']) && !empty($_GET['file'])) {
				/*
				 * 此行是何作用？如果目录中含有php字样则会把文件路径编程无效路径，因此去掉php的替换
				 * jiaokun,20141120
				 */
			    //$_GET['file'] = str_ireplace(array(';','php'),'',$_GET['file']);
			    $_GET['file'] = str_ireplace(array(';'),'',$_GET['file']);
				if(is_image($_GET['file'])== false || stripos($_GET['file'],'.php')!==false) exit();
				bc_base::load_sys_func('dir');
				if (strpos($_GET['file'], bc_base::load_config('system', 'upload_url'))!==false) {
					$file = $_GET['file'];
				...
				$filepath = date('Y/md/');
				dir_create($this->upload_path.$filepath); //Fix code bug, add by jiaokun, 20141120
				file_put_contents($this->upload_path.$filepath.$new_file, $pic);
			} else {
				return false;
			}
			echo bc_base::load_config('system', 'upload_url').$filepath.$new_file;
			exit;
		}
	}
	...
	public function swfdelete() {
		$attachment = bc_base::load_sys_class('attachment');
	...
	public function album_load() {
		...
		$where = $uploadtime= '';
		$this->att_db= bc_base::load_model('attachment_model');
	...
	public function album_dir() {
		...
		$local = str_replace(array(BC_PATH, BANGCMS_PATH ,DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('','',DIRECTORY_SEPARATOR), $filepath);
	...
	final public static function admin_tpl($file, $m = '') {
		...
		return BC_PATH.'modules'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
	}
	...
	public function webuploader()
	{
		$grouplist = getcache('grouplist', 'member');
		if (isset($_POST['dosubmit'])) {
			if ($_POST['swf_auth_key'] != md5(bc_base::load_config('system', 'auth_key') . $_POST['SWFUPLOADSESSID']) || ($_POST['isadmin'] == 0 && !$grouplist[$_POST['groupid']]['allowattachment'])) exit('403');
			bc_base::load_sys_class('attachment', '', 0);
			$attachment = new attachment($_POST['module'], $_POST['catid'], $_POST['siteid']);
			$attachment->set_userid($_POST['userid']);
			$siteid = get_siteid();
			...
			$siteid = $this->get_siteid();
			$sess_id = SYS_TIME;
			$swf_auth_key = md5(bc_base::load_config('system', 'auth_key') . $sess_id);
			$userid_flash = sys_auth($this->userid, 'ENCODE');
			$site_setting = get_site_setting($siteid);
			$_args = explode(',',$args);
			...
			$admin_url = bc_base::load_config('system', 'admin_url');
