...
<div class="table-list">
    <table width="100%" cellspacing="0" >
        <thead>
            ...
            <th width="75"><?php echo L('listshow');?></th>
            <th width="75"><?php echo L('listfilter');?></th>
	...
	foreach($datas as $r) {
		...
		$setting_listshow = false;
		$setting_listfilter = false;
		$setting_arr = string2array($r['setting']);
		if ( $setting_arr['listshow'] || in_array( $r['field'], get_default_show() ) ) {
			$setting_listshow = true;
		}
		if ( $setting_arr['listfilter'] || in_array( $r['field'], get_default_listfilter_show() ) ) {
			$setting_listfilter = true;
		}
	?>
    <tr>
		...
		<td width="75" align='center'><?php echo $setting_listshow ? L('icon_unlock') : L('icon_locked')?></td>
		<td width="75" align='center'><?php echo $setting_listfilter ? L('icon_unlock') : L('icon_locked')?></td>
		<td align='center'> <a href="?m=content&c=sitemodel_field&a=edit&modelid=<?php echo $r['modelid']?>&fieldid=<?php echo $r['fieldid']?>&menuid=<?php echo $_GET['menuid']?>"><?php echo L('edit');?></a> | 
			...
			<a href="javascript:confirmurl('?m=content&c=sitemodel_field&a=delete&modelid=<?php echo $r['modelid']?>&fieldid=<?php echo $r['fieldid']?>&menuid=<?php echo $_GET['menuid']?>','<?php echo L('confirm',array('message'=>$r['name']))?>')"><?php echo L('delete')?></a><?php } else {?><font color="#BEBEBE"> <?php echo L('delete');?></font><?php }?> 
			</td>
	</tr>
	<?php } ?>
    </tbody>
