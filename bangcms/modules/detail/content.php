...
class content extends admin {
	...
	public function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('detail_model');
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			$parent_model_id = intval($_GET['parent_model_id']);
			$relation_model = intval($_GET['relation_model']);
			$relation_field = $_GET['relation_field'];
			$pid = intval($_GET['pid']);
			//todo:701
			$relation_par = "&parent_model_id=$parent_model_id&relation_model=$relation_model&relation_field=$relation_field&pid=$pid";
			...
			if(isset($_GET['keyword']) && !empty($_GET['keyword'])) {
				...
				if ( is_numeric($_GET['searchtype']) ) {
					...
					$search_arr = explode( ',', strip_tags(trim($_GET['searchtype'])) );
					$searchtype = $search_arr['0'];
					$searchfeild = $search_arr['1'];
					$alias = ($search_arr['2']==0) ? '`s`.' : '' ;
				}
				switch ($searchtype) {
					case '0':
					case '1':
					case '2':
						$where .= " AND `$type_array[$searchtype]` like '%$keyword%'";
						break;
					case '3':
					...
					case 'textarea':
					case 'box':
						$where .= " AND ".$alias."`$searchfeild` like '%$keyword%'";
						break;
					case 'linkage':
						break;
					default:
			...
			//取出listshow、listfilter、onetomany字段,并标记是否需取附属表信息
			$list_ext_field = $this->get_list_ext_field($model_fields);
			$listinfo_enough = $list_ext_field['listinfo_enough'];
			$list_show_fields = $list_ext_field['list_show_fields'];
			$list_filter_fields = $list_ext_field['list_filter_fields'];
			$onetomany_fields = $list_ext_field['onetomany_fields'];
			
			if ($listinfo_enough) {
				$datas = $this->db->listinfo($where,'posids desc,listorder desc,id desc',$_GET['page']);
			} else {
				$datas = $this->db->listinfoall($where,'posids desc,listorder desc,m.id desc',$_GET['page']);//取出包含附属表的全部信息
			}
			$pages = $this->db->pages;
			...
			for($i=1;$i<=$workflow_steps;$i++) {
				...
				$newimg = $r ? '<img src="'.IMG_PATH.'icon/new.png" style="padding-bottom:2px" onclick="window.location.href=\'?m=detail&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&pc_hash='.$pc_hash.'\'">' : '';
				$workflow_menu .= '<a href="?m=detail&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&pc_hash='.$pc_hash.'" '.$current.' ><em>'.L('workflow_'.$i).$newimg.'</em></a><span>|</span>';
			}
			if($workflow_menu) {
				$current = isset($_GET['reject']) ? 'class=on' : '';
				$workflow_menu .= '<a href="?m=detail&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&pc_hash='.$pc_hash.'&reject=1" '.$current.' ><em>'.L('reject').'</em></a><span>|</span>';
			}
	...
	public function add() {
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			$relation_par = "&parent_model_id=$parent_model_id&relation_model=$relation_model&relation_field=$relation_field&pid=$pid";
			//处理关联字段，设置pid
			if ( isset($_POST['relation_field']) && isset($_POST['pid']) ) {
				$_POST['info'][$_POST['relation_field']] = $_POST['pid'];
			}
			
			if($category['type']==0) {
				//$modelid = $this->categorys[$catid]['modelid'];
				//todo:701
				$modelid = intval($_GET['relation_model']);
				...
				//单网页
				$this->page_db = bc_base::load_model('page_model');
	...
	public function delete() {
		if(isset($_GET['dosubmit'])) {
			...
			$this->db->set_model($modelid);
			$this->hits_db = bc_base::load_model('hits_model');
			...
			//附件初始化
			$attachment = bc_base::load_model('attachment_model');
			$this->content_check_db = bc_base::load_model('content_check_model');
			$this->position_data_db = bc_base::load_model('position_data_model');
			$this->search_db = bc_base::load_model('search_model');
			//判断视频模块是否安装 
			if (module_exists('video') && file_exists(BC_PATH.'model'.DIRECTORY_SEPARATOR.'video_content_model.class.php')) {
				$video_content_db = bc_base::load_model('video_content_model');
				$video_install = 1;
			}
			$this->comment = bc_base::load_app_class('comment', 'comment');
			$search_model = getcache('search_model_'.$this->siteid,'search');
			$typeid = $search_model[$modelid]['typeid'];
			$this->url = bc_base::load_app_class('url', 'content');
	...
	public function pass() {
		...
		if($workflowid) {
			...
			//更改内容状态
				...
				$modelid = intval($_GET['relation_model']);
				$this->db->set_model($modelid);
				$this->db->search_db = bc_base::load_model('search_model');
				//审核通过，检查投稿奖励或扣除积分
				if ($status==99) {
					$html = bc_base::load_app_class('html', 'content');
					$this->url = bc_base::load_app_class('url', 'content');
					...
					if (isset($_POST['ids']) && !empty($_POST['ids'])) {
						foreach ($_POST['ids'] as $id) {
							...
							if($setting['presentpoint']>0) {
								bc_base::load_app_class('receipts','pay',0);
								...
								bc_base::load_app_class('spend','pay',0);
		...
		if(!empty($this->categorys)) {
			foreach($this->categorys as $r) {
				...
				if($r['type']==1 || $from=='block') {
					...
					$r['type'] = 'init';
					$r['add_icon'] = "<a target='right' href='?m=detail&c=content&menuid=".$_GET['menuid']."&catid=".$r['catid']."' onclick=javascript:openwinx('?m=detail&c=content&a=add&menuid=".$_GET['menuid']."&catid=".$r['catid']."&hash_page=".$_SESSION['hash_page']."','')><img src='".IMG_PATH."add_content.gif' alt='".L('add')."'></a> ";
				}
				$categorys[$r['catid']] = $r;
			}
		}
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					...
					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?m=detail&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right' onclick='open_list(this)'>\$catname</a></span>";
						$strs2 = "<span class='folder'>\$catname</span><a class='self' href='?m=detail&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right' onclick='open_list(this)'>【 \$catname 】内容</a>";
						break;
				}
	...
	public function public_preview() {
		...
		if($rs['paginationtype']==1) {
			...
			$contentpage = bc_base::load_app_class('contentpage');
		...
		function ajax_manage(type) {
			if(type==1) {
				$.get('?m=detail&c=content&a=pass&ajax_preview=1&catid=".$catid."&steps=".$steps."&id=".$id."&pc_hash=".$pc_hash."');
			} else if(type==2) {
				$.get('?m=detail&c=content&a=pass&ajax_preview=1&reject=1&catid=".$catid."&steps=".$steps."&id=".$id."&pc_hash=".$pc_hash."');
			} else if(type==3) {
				$.get('?m=detail&c=content&a=delete&ajax_preview=1&dosubmit=1&catid=".$catid."&steps=".$steps."&id=".$id."&pc_hash=".$pc_hash."');
			}
	...
	public function public_getsite_categorys() {
		...
		foreach($this->categorys as $r) {
			...
			if ($_GET['modelid'] && $r['modelid']!=$_GET['modelid']) {
				continue;
			}
			$r['modelname'] = $models[$r['modelid']]['name'];
			$r['style'] = $r['child'] ? 'color:#8A8A8A;' : '';
			$r['click'] = $r['child'] ? '' : "onclick=\"select_list(this,'".safe_replace($r['catname'])."',".$r['catid'].")\" class='cu' id='catid_v$r[catid]' title='".L('click_to_select')."'";
			$categorys[$r['catid']] = $r;
		}
		$str  = "<tr \$click >
		...
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					...
					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?m=detail&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid&pc_hash=".$_SESSION['pc_hash']."' target='right' onclick='open_list(this)'>\$catname</a></span>";
						break;
				}
						...
						if ($t=='content') {
							...
							foreach ($model as $modelid) {
								...
								if ($r = $db->count()) { //判断模型下是否有数据
									...
									$db->query('TRUNCATE TABLE `bangcms_'.$model_arr[$modelid]['tablename'].'`');
									//开始清理模型data表数据
									...
									$db->query('TRUNCATE TABLE `bangcms_'.$model_arr[$modelid]['tablename'].'_data`');
									//删除该模型中在hits表的数据
									$hits_db = bc_base::load_model('hits_model');
									$hitsid = 'c-'.$modelid.'-';
									...
									//删除该模型在search中的数据
									$search_db = bc_base::load_model('search_model');
									...
									$position_db = bc_base::load_model('position_data_model');
									...
									//附件初始化
									//$attachment = bc_base::load_model('attachment_model');
									//$comment = bc_base::load_app_class('comment', 'comment');
									//if(module_exists('comment')){
										//$comment_exists = 1;
									//}
									//foreach ($data as $d) {
							...
							$comment_db = bc_base::load_model('comment_data_model');
							for($i=1;;$i++) {
								...
								if ($comment_db->table_exists(str_replace($comment_db->db_tablepre, '', $comment_db->table_name))) {
									if ($r = $comment_db->count()) {
										...
										$comment_db->query('TRUNCATE TABLE `bangcms_comment_data_'.$i.'`');
									}
								} else {
									break;
								}
							...
							$db = bc_base::load_model($t.'_model');
							if ($r = $db->count()) {
								$result = $db->select();
								...
								$db->query('TRUNCATE TABLE `bangcms_'.$t.'`');
							}
						}
					}
				}
	...
	/**
	 * 取出自定义显示字段、筛选字段、一对多字段,并标记是否需取附属表信息
	 * @param array $model_fields 模型信息
	 * @return array (
	 *	'listinfo_enough'=>boolean 是否需取附属表信息,
	 *	'list_show_fields'=>array() 自定义显示字段,
	 *	'list_filter_fields'=>array() 自定义过滤字段,
	 *	'onetomany_fields'=>array() 自定义一对多字段,
	 * )
	 */
	private function get_list_ext_field($model_fields){
		$listinfo_enough = true;
		$list_show_fields = array();
		$list_filter_fields = array();
		$onetomany_fields = array();
		foreach ($model_fields as $field_v) {
			if ($field_v['formtype']=='onetomany') {
				array_push(
					$onetomany_fields,
					array(
						'parent_model_id' => $MODEL['modelid'],
						'formtype' => $field_v['formtype'],
						'field' => $field_v['field'],
						'name' => $field_v['name'],
						'relation_model' => $field_v['relation_model'],
						'relation_field' => $field_v['relation_field'],
					)
				);
					
				if ($listinfo_enough) {
					if ( $field_v['issystem']==0 ) {
						$listinfo_enough = false;
					}
				}
			}
			if ($field_v['listshow']==1) {
				$temp_['formtype'] = $field_v['formtype'];
				$temp_['field'] = $field_v['field'];
				$temp_['name'] = $field_v['name'];
				$temp_['boxtype'] = $field_v['boxtype'];
				$temp_['fieldtype'] = empty($field_v['fieldtype']) ?  'int': $field_v['fieldtype'];
				$temp_['format'] = $field_v['format'];
				$temp_['linkageid'] = $field_v['linkageid'];
				$temp_['defaultvalue'] = $field_v['defaultvalue'];
					
				if ($field_v['formtype']=='box') {
					$options = explode("\n",$field_v['options']);
					...
					$temp_['options'] =$option;
				}
				array_push($list_show_fields,$temp_);
			...
			if ($field_v['listfilter']==1) {
				$temp__['formtype'] = $field_v['formtype'];
				$temp__['field'] = $field_v['field'];
				$temp__['name'] = $field_v['name'];
				$temp__['boxtype'] = $field_v['boxtype'];
				$temp__['linkageid'] = $field_v['linkageid'];
				$temp__['issystem'] = $field_v['issystem'];
				...
				if ($field_v['formtype']=='box') {
					...
					$temp__['options'] =$option;
				}
				array_push($list_filter_fields,$temp__);
		...
		return array(
				'listinfo_enough'=>$listinfo_enough,
				'list_show_fields'=>$list_show_fields,
				'list_filter_fields'=>$list_filter_fields,
				'onetomany_fields'=>$onetomany_fields,
		);
	}
}
?>