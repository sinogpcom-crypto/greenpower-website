...
class html {
	...
	public function __construct() {
		...
		$this->html_root = bc_base::load_config('system','html_root');
		...
		if($paginationtype!=0) {
			...
			if($CONTENT_POS !== false) {
				...
				foreach ($pageurls as $page=>$urls) {
					...
					$pagefile = BANGCMS_PATH.$pagefile;
					ob_start();
	...
	public function category($catid, $page = 0) {
		...
		if($create_to_html_root) {
			if($this->siteid==1) {
				...
				$file = BANGCMS_PATH.substr($this->html_root,1).$base_file;
			}
			//添加到发布点队列
