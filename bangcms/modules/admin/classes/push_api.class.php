...
class push_api {
	...
	public function __construct() {
		$this->db = bc_base::load_model('position_model');  //加载数据模型
	}
	 ...
	 * 调用方式
	 * $push = bc_base::load_app_class('push_api','admin');
	...
	public function position_update($id, $modelid, $catid, $posid, $data, $expiration = 0, $undel = 0, $model = 'content_model') {
		...
		$param[0] = $data;
		$param[0]['id'] = $id;
		if ($undel==0) $pos_info = $this->position_del($catid, $id, $posid, $modelid);
	 ...
	 * @param int $id 文章id
	 * @param array $input_posid 传入推荐位数组 $modelid
	 * @param array $modelid 模型id
	 */
	private function position_del($catid,$id,$input_posid,$modelid=0) {
		$array = array();
		$pos_data = bc_base::load_model('position_data_model');

		//查找已存在推荐位
		if($modelid) {
			$r = $pos_data->select(array('id'=>$id,'catid'=>$catid,'modelid'=>$modelid),'posid');
		} else {
		...
		if($modelid) {
			$sql = "`catid`='$catid' AND `id`='$id' AND `modelid`=".$modelid." AND `posid` IN ($real_posid)";
		} else {
	...
	private function content_pos($id, $modelid) {
		...
		$modelid = intval($modelid);
		if ($id && $modelid) {
			$db_data = bc_base::load_model('position_data_model');
			$this->db_content = bc_base::load_model('content_model');
			...
			if (is_array($arr['posid']) && !empty($arr['posid']) && is_array($param) && !empty($param)) {
				foreach ($arr['posid'] as $pid) {
					...
					foreach ($param as $d) {
						...
						//推荐纪录为1:n关联模块的n
						if(!empty($d['n_relation_field']) && !empty($d['n_pid']) ) {
							$info['data']['n_relation_field'] = $d['n_relation_field'];
							$info['data']['n_pid'] = $d['n_pid'];
						}
						unset($fields_arr);
						unset($fields_value);
						...
						if ($r = $pos_data->get_one(array('id'=>$d['id'], 'posid'=>$pid, 'catid'=>$info['catid'], 'modelid'=>$modelid))) {
							if($r['synedit'] == '0') $pos_data->update($info, array('id'=>$d['id'], 'posid'=>$pid, 'catid'=>$info['catid'], 'modelid'=>$modelid));
						} else {
					...
					$maxnum = $position_info[$pid]['maxnum'];
					$r = $pos_data->select(array('catid'=>$catid, 'posid'=>$pid, 'modelid'=>$modelid), 'id, listorder', $maxnum.',1', 'listorder DESC, id DESC');
					if ($r && $position_info[$pid]['maxnum']) {
						...
						foreach ($result as $r) {
							$pos_data->delete(array('id'=>$r['id'], 'posid'=>$pid, 'catid'=>$catid, 'modelid'=>$modelid));
