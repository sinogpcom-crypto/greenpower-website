...
/**
 * $Author: jiaokun $
 * $Id: admin.class.php 80 2014-10-31 07:12:53Z jiaokun $
 * $Date: 2014-10-31 15:12:53 +0800 (Fri, 31 Oct 2014) $
 * 
 * */
...
class admin {
	...
	public function __construct() {
		...
		self::lock_screen();
		self::check_hash();
		if(bc_base::load_config('system','admin_url') && $_SERVER["HTTP_HOST"]!= bc_base::load_config('system','admin_url')) {
	...
	final public static function admin_menu($parentid, $with_self = 0) {
		...
		$array = array();
		$privdb = bc_base::load_model('admin_role_priv_model');
		...
		extract($setconfig);
 		if($admin_log==1){
 			...
 			if($action == '' || strchr($action,'public') || $action == 'init' || $action=='public_current_pos') {
				...
				$ip = ip();
				$log = bc_base::load_model('log_model');
	...
	final private function check_ip(){
		$this->ipbanned = bc_base::load_model('ipbanned_model');
	...
	final public function admin_list_template($id = '', $str = '') {
		$templatedir = BC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR;
		$pre = 'content_list';
