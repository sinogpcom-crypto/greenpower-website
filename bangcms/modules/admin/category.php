...
class category extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('category_model');
	...
	public function init () {
		...
		if(!empty($result)) {
			foreach($result as $r) {
				...
				$r['str_manage'] .= ' | <a href="?m=admin&c=category&a=add_test_data&parentid='.$r['catid'].'&pc_hash='.$_SESSION['pc_hash'].'">'.L('add_test_data').'</a>';
	...
	/**
	 * 添加测试数据，内部栏目每轮添加5条，单网页如果没有数据添加一条
	 * 谢亚南  2015-01-08
	 */
	public function add_test_data(){
		if(!isset($_GET['parentid'])||!is_numeric(intval($_GET['parentid']))){
			showmessage(L('add_test_data_failure'),HTTP_REFERER);
			exit;
		}else{
			$catid = intval($_GET['parentid']);
			//获取栏目相关信息
			$data = $this->db->get_one(array('catid'=>$catid),'type,modelid,catname,usable_type');
			//类型:单网页-1     内部栏目-0
			if($data['type']==1){
				$this -> page_add_test($catid,$data['catname']);
				
			}else if($data['type']==0){
				$this -> column_add_test($catid,$data);
			}
		}		
	}
	 ...
	 * 单网页添加测试数据
	 */
	public function page_add_test($catid,$catname){	 
		
		//存在数据，直接返回
		$page_data = $this -> db -> query('SELECT title FROM `bc_page` WHERE catid='.$catid); 
		$page_data = $this->db->fetch_array($page_data);
		if(count($page_data)){
			showmessage(L('page_existing_data'),HTTP_REFERER);
			exit;
		}
		...
		$page_data = array();
		$page_data['catid'] = $catid;
		$page_data['title'] = $catname.'单网页';
		$page_data['keywords'] = $catname;
		$page_data['content'] = '<p>';
		for($i=0;$i<100;$i++){
			$page_data['content'] .= $page_data['title'].rand();
			if($i%10==0&&$i!=0){
				$page_data['content'] .='<br/>';
			}
		}
		$page_data['content'] .='</p>';
		$page_data['updatetime'] = time();
		$page_data['style'] = ';';
		$result = $this->page_db->insert($page_data,1);
		//修改栏目的url
		$setting = $this->db->get_one(array('catid'=>$catid),'setting');
		...
		if($cat['url']!=$url) $result = $this->db->update(array('url'=>$url), array('catid'=>$catid));
		
		if($result){
			showmessage(L('add_single_success'),HTTP_REFERER);
			$this->page_db->create_html($catid,$page_data);
		}else{
			showmessage(L('add_single_failure'),HTTP_REFERER);
			exit;
		}
	} 
	/**
	...
	public function column_add_test($catid,$data){
		
		$test_http = bc_base::load_sys_class('http');
		$this->content_db = bc_base::load_model('content_model');
		$this->content_db->set_model($data['modelid']);

		//查询模型字段
		$model_field = $this -> db -> query('SELECT field,maxlength,formtype,issystem,setting FROM `bc_model_field` WHERE modelid='.$data['modelid'].' AND disabled=0 AND formtype not in ("datetime","posid","author","copyfrom","groupid","islink","template","pages","readpoint","linkage","downfile","map","omnipotent") AND field not in ("inputtime","updatetime","status","url","listorder","allow_comment","username","issystem")');	
		$model_field = $this -> db ->fetch_array($model_field);	
		
		//得到远程测试数据，加密密码 key = bangcms20150108		
		$is_ok = $test_http->post(bc_base::load_config('system','test_data_path'), array("authkey"=>'zbbangcms@'));
		$test_data = array();
		if ( $is_ok ) {
			$test_data = unserialize( gzinflate( ltrim(base64_decode( $test_http->get_data()) ) ) );
		}
		foreach($test_data as $d){
			//拼接data
			$info = array('status'=>99);
			$info_data = array();
			foreach($model_field as $f){
				switch($f['field']){
					case 'catid':{
					...
					case 'typeid':{
							$type_id = $type_id_arr[mt_rand(0,count($type_id_arr)-1)];
							if($type_id){
								$info['typeid'] = $type_id;
							}
						break;
					...
					default:{
						switch($f['formtype']){
							case 'box':{
									$box_setting = string2array($f['setting']);
									$box_value = explode("\n",$box_setting['options']);
									$box = array();
									foreach ($box_value as $v) {
										$value = explode("|",$v);
										array_push($box,trim($value[1]));
									}
									if($box_setting['boxtype']=='checkbox'){
										$box_temp = array('-99',$box[mt_rand(0,count($box_value)-1)]);
									}else if($box_setting['boxtype']=='multiple'){
										$box_temp = array($box[mt_rand(0,count($box_value)-1)]);
									}else{
										$box_temp = $box[mt_rand(0,count($box_value)-1)];
									}
										$info[$f['field']] = $box_temp;
								break;
							}
							case 'number':{
									$num_setting = string2array($f['setting']);
									$min = isset($num_setting['minnumber'])?$num_setting['minnumber']:0;
									$max = isset($num_setting['maxnumber'])?$num_setting['maxnumber']:100;
									$info[$f['field']] = mt_rand($min,$max-1);
								break;
							}
							case 'downfiles':{
									$setting = string2array($f['setting']);
									if(is_numeric(strpos(strtolower($setting[upload_allowext]),'mp4'))){
										if($d['video']){
											$info_data[$f['field']] = trim($d['video'],"\'");
											$flag = 1;
										}
									}else{
										if($d['downfiles']){
											$info_data[$f['field']] = trim($d['downfiles'],"\'");
											$flag = 1;
										}
									}
								break;
							...
							default:{
									$form_temp = $d[$f['formtype']];
									if($form_temp){
										if($f['maxlength']>0){
											$info[$f['field']] = mb_substr($form_temp,0,$f['maxlength']);
										}else{
											$info[$f['field']] = $form_temp;
										}
									}else{
										$info[$f['field']] = 'This Is NUll';
									}
								break;
							}
						}
			...
			$res = $this->content_db->add_content($info);	
			if(!$res){
				showmessage(L('add_test_data_failure'),HTTP_REFERER);
				exit;
			}
			if($flag==1){
				$sql = 'UPDATE '.$this->content_db->table_name.' SET ';
		...
		showmessage(L('add_test_data_success_5'),HTTP_REFERER);
	}
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['template_child']){
                                ...
                                $idstr = $this->get_arrchildid($catid);
                                 if(!empty($idstr)){
                                        $sql = "select catid,setting from bc_category where catid in($idstr)";
                                        $this->db->query($sql);
			...
			$this->cache();
			//更新附件状态
			if($_POST['info']['image'] && bc_base::load_config('system','attachment_stat')) {
				$this->attachment_db = bc_base::load_model('attachment_model');
	...
	private function delete_category_video($catid, $modelid) {
		$content_model = bc_base::load_model('content_model');
	...
	*/
	private function update_url($catid) {
		...
		$url = bc_base::load_app_class('url', 'content'); //调用URL实例
	...
	public function batch_edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			foreach ($infos as $catid=>$info) {
				...
				//更新附件状态
				if($info['image'] && bc_base::load_config('system','attachment_stat')) {
