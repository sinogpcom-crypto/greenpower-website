...
class role extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('admin_role_model');
		$this->priv_db = bc_base::load_model('admin_role_priv_model');
		$this->op = bc_base::load_app_class('role_op');
	}
	...
	public function role_priv() {
		$this->menu_db = bc_base::load_model('menu_model');
		...
		if(isset($_POST['dosubmit'])){
			...
			$roleid = intval($_GET['roleid']);
			if ($siteid) {
				$menu = bc_base::load_sys_class('tree');
				$menu->icon = array('│ ','├─ ','└─ ');
				$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
	...
	public function priv_setting() {
		$sites = bc_base::load_app_class('sites', 'admin');
	...
	public function member_manage() {
		$this->admin_db = bc_base::load_model('admin_model');
		$roleid = intval($_GET['roleid']);
	...
	public function setting_cat_priv() {
		...
		switch ($op) {
			...
			bc_base::load_app_class('role_cat', '', 0);
			...
			$show_header = true;
			$str = "<tr>
					<td align='center'><input type='checkbox' class='loves'  value='1' onclick='select_all(\$catid, this)' ></td>
				  <td>\$spacer\$catname</td>
				  <td align='center'><input type='checkbox' class='loves' name='priv[\$catid][]' \$init_check  value='init' ></td>
				  <td align='center'><input type='checkbox' class='loves' name='priv[\$catid][]' \$disabled \$add_check value='add' ></td>
				  <td align='center'><input type='checkbox' class='loves' name='priv[\$catid][]' \$disabled \$edit_check value='edit' ></td>
				  <td align='center'><input type='checkbox' class='loves' name='priv[\$catid][]' \$disabled \$delete_check  value='delete' ></td>
				  <td align='center'><input type='checkbox' class='loves' name='priv[\$catid][]' \$disabled \$listorder_check value='listorder' ></td>
				  <td align='center'><input type='checkbox' class='loves' name='priv[\$catid][]' \$disabled \$push_check value='push' ></td>
				  <td align='center'><input type='checkbox' class='loves' name='priv[\$catid][]' \$disabled \$move_check value='remove' ></td>
			  </tr>";
