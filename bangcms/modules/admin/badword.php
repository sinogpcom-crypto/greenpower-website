...
class badword extends admin {
	function __construct() {
		...
		$this->db = bc_base::load_model('badword_model');
		parent::__construct();
	}
	...
	public function public_name() {
			$badword = isset($_GET['badword']) && trim($_GET['badword']) ? (bc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['badword'])) : trim($_GET['badword'])) : exit('0');
