...
class database extends admin {
	...
	function __construct() {
		...
		bc_base::load_sys_class('db_factory');
	...
	public function import() {
		$database = bc_base::load_config('database');
		if($_GET['dosubmit']) {
			$admin_founders = explode(',',bc_base::load_config('system','admin_founders'));
			...
			if(is_array($sqlfiles)) {
				...
				foreach($sqlfiles as $id=>$sqlfile) {
					//老的数据库备份文件转换为新格式
					if(preg_match("/([bangcmstables_|db_][0-9]{8}_[0-9a-z]{20}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
					...
					if(preg_match("/([0-9a-z]{20}_[bangcmstables_|db_]+[0-9]{8}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
	...
	private function status($tables,$tablepre) {
		...
		return array('bangcmstables'=>$bangcms, 'othertables'=>$other);
	}
	 ...
	 * @param unknown_type $tabletype 备份数据库类型 （非bangcms数据与bangcms数据）
	 */
	private function export_database($tables,$sqlcompat,$sqlcharset,$sizelimit,$action,$fileid,$random,$tableid,$startfrom,$tabletype) {
		...
		if(trim($tabledump)) {
			$tabledump = "# bangcms bakfile\n# version:BANGCMS\n# time:".date('Y-m-d H:i:s')."\n# type:bangcms\n# bangcms:http://www.zhengbang.com.cn\n# --------------------------------------------------------\n\n\n".$tabledump;
			$tableid = $i;
	 ...
	 */
 	...
 	private function sql_split($sql) {
		...
		if($this->db_tablepre != "bangcms_") $sql = str_replace("`bangcms_", '`'.$this->db_tablepre, $sql);
		$sql = str_replace("\r", "\n", $sql);
		$ret = array();
		$num = 0;
