...
?>   
	...
	<?php 
	if ($info['is_relation_model']) {
		$edit_url = '?m=detail&c=content&a=edit&catid='.$info['catid'].'&id='.$info['id'].'&parent_model_id=0&relation_model='.$info['modelid'].'&relation_field='.$info['n_relation_field'].'&pid='.$info['n_pid'];
	} else{
		$edit_url = '?m=content&c=content&a=edit&catid='.$info['catid'].'&id='.$info['id'];	
	}
	?>
	<td width="15%" align="center"><a href="<?php echo $info['url']?>" target="_blank"><?php echo L('posid_item_view')?></a> | 
	<a onclick="javascript:openwinx('<?php echo $edit_url?>','')" href="javascript:;"><?php echo L('posid_item_edit');?></a> | <a href="javascript:item_manage(<?php echo $info['id']?>,<?php echo $info['posid']?>, <?php echo $info['modelid']?>,'<?php echo $info['title']?>')"><?php echo L('posid_item_manage')?></a>
	</td>
	</tr>
<?php 
	}
