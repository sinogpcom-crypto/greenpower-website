...
<div class="header">
	<div class="logo lf"><a href="<?php echo $currentsite['domain']?>" target="_blank"><span class="invisible"><?php echo L('bangcms_title')?></span></a></div>
    <div class="col-auto">
    	<div class="log white cut_line"><?php echo L('hello'),$admin_username?>  [<?php echo $rolename?>]<span>|</span><a href="?m=admin&c=index&a=public_logout">[<?php echo L('exit')?>]</a><span>|</span>
    		<a href="/index.php" target="_blank" id="site_homepage"><?php echo L('site_homepage')?></a><span>|</span>
    		<!-- <a href="?m=member" target="_blank"><?php echo L('member_center')?></a><span>|</span> -->
    		 <a href="/uploadfile/file/后台操作文档.zip" target="_blank">后台文档下载</a><span>|</span> 
        ...
        foreach($array as $_value) {
        	...
        	} else if($_value['id'] == 1589){
        		echo '<li id="_M'.$_value['id'].'" class="top_menu flag_menu"><div class="flag">1</div><a href="javascript:_M('.$_value['id'].',\'?m='.$_value['m'].'&c='.$_value['c'].'&a='.$_value['a'].'\')"  hidefocus="true" style="outline:none;">'.L($_value['name']).'</a></li>';
        	} else {
...
function show_help(targetUrl) {
	...
	$("#help").slideUp("slow");
	var str = '';
	$.getJSON("http://netbang.zhengbang.net/api.php?jsoncallback=?",{op:'help',targetUrl: targetUrl},
	function(data){
		if(data!=null) {
			$("#help").slideDown("slow");
