<?php 
	...
	function template_list($siteid = '', $disable = 0) {
		$list = glob(BC_PATH.'templates'.DIRECTORY_SEPARATOR.'*', GLOB_ONLYDIR);
		$arr = $template = array();
		if ($siteid) {
	...
	function set_config($config, $filename="system") {
		...
		return bc_base::load_config('system','lock_ex') ? file_put_contents($configfile, $str, LOCK_EX) : file_put_contents($configfile, $str);		
	}
	...
	function errorlog_size() {
		...
		if(file_exists($logfile)) {
			return $logsize = bc_base::load_config('system','errorlog') ? round(filesize($logfile) / 1048576 * 100) / 100 : 0;
		} 
		return 0;
	}
