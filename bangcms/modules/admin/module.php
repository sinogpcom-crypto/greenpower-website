...
class module extends admin {
	...
	public function init() {
		...
		$dirs = glob(BC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.'*');
		foreach ($dirs as $d) {
			if (is_dir($d)) {
				$d = basename($d);
				$dirs_arr[] = $d;
	...
	public function install() {
		...
		$module_api = bc_base::load_app_class('module_api');
		...
		if ($_POST['dosubmit']) {
			...
			include BC_PATH.'modules'.DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'install'.DIRECTORY_SEPARATOR.'config.inc.php';
