...
class admin_manage extends admin {
	...
	function __construct() {
		...
		$this->op = bc_base::load_app_class('admin_op');
	}
	...
	public function public_edit_info() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$lang_dirs = glob(BC_PATH.'languages/*');
			$dir_array = array();
			foreach($lang_dirs as $dirs) {
				$dir_array[] = str_replace(BC_PATH.'languages/','',$dirs);
			}
	...
	//电子口令卡
	function card() {
		if (bc_base::load_config('system', 'safe_card') != 1) {
