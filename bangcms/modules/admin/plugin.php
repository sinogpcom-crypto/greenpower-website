...
bc_base::load_sys_func('plugin');
class plugin extends admin {
	private $db,$db_var;
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('plugin_model');
		$this->db_var = bc_base::load_model('plugin_var_model');
	...
	public function init() {
		...
		if($pluginfo = $this->db->select('','*','','disable DESC,listorder DESC')) {
			foreach ($pluginfo as $_k=>$_r) {
				if(file_exists(BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$_r['dir'].DIRECTORY_SEPARATOR.$_r['dir'].'.class.php')){
	...
	public function import() {
		if(!isset($_GET['dir'])) {
			...
			$pluginsdir = dir(BC_PATH.'plugin');
			while (false !== ($entry = $pluginsdir->read())) {
				$config_file = '';
				$plugin_data = array();
				if(!in_array($entry, array('.', '..')) && is_dir(BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$entry) && !in_array($entry, $installsdir) && !$this->db->get_one(array('identification'=>$entry))) {
					$config_file = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$entry.DIRECTORY_SEPARATOR.'plugin_'.$entry.'.cfg.php';
			...
			$dir = trim($_GET['dir']);
			$license = 0;
			$config_file = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.'plugin_'.$dir.'.cfg.php';
			if(file_exists($config_file)) {
				$plugin_data = @require($config_file);
	...
	public function delete() {
		if(isset($_POST['dosubmit'])) {
			...
			$filename = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.$plugin_data['plugin']['uninstallfile'];
			...
			if($op_status) {
				...
				if($plugin_data['plugin']['iframe']) {
					...
					if(!dir_delete(BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir)) {
	...
	public function install() {
		...
		/*
		 * 插件合法性验证，del by jiaokun,20141124
		 */
		/*
		...
		*/
		if($plugin_data['version'] && $plugin_data['version']!=bc_base::load_config('version', 'bc_version')) {
		...
		$filename = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$dir.DIRECTORY_SEPARATOR.$plugin_data['plugin']['installfile'];
		...
		if($op_status) {	
			...
			//插件安装量统计,del by jiaokun,20141124		
	...
	public function design() {
	    ...
	    if(isset($_POST['dosubmit'])) {
			...
			$filepath = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$data['identification'].DIRECTORY_SEPARATOR.'plugin_'.$data['identification'].'.cfg.php';
			bc_base::load_sys_func('dir');
			dir_create(dirname($filepath));	
		    $data = "<?php\nreturn ".var_export($data, true).";\n?>";			
			if(bc_base::load_config('system', 'lock_ex')) {
	...
	public function appcenter() {
		...
		$data = file_get_contents('http://open.zhengbang.com.cn/index.php?m=open&c=api&a=get_applist&s='.$s.'&p='.$p);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		
		$recommed_data = file_get_contents('http://open.zhengbang.com.cn/index.php?m=open&c=api&a=get_recommed_applist&s=5&p=1');
		$recommed_data = array_iconv(json_decode($recommed_data, true),'utf-8',CHARSET);
		
		$focus_data = file_get_contents('http://open.zhengbang.com.cn/index.php?m=open&c=api&a=get_app_focus&num=3');
	...
	public function appcenter_detail() {
		$data = array();
		$id = intval($_GET['id']);
		$data = file_get_contents('http://open.zhengbang.com.cn/index.php?m=open&c=api&a=get_detail_byappid&id='.$id);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		extract($data);		
		if($appname) {
	...
	public function install_online() {
		...
		if(!empty($data['iframe'])) {
			$appdirname = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$data['appenname'];
...
EOF;
			...
			//解压缩
			bc_base::load_app_class('pclzip', 'upgrade', 0);
			...
			//动态程序路径
			$copy_to_pcpath = BC_PATH.'plugin';
			//静态程序路径
			$copy_to_staticspath = BANGCMS_PATH.'statics'.DIRECTORY_SEPARATOR.'plugin';

			//应用文件夹名称
			$appdirname = $data['appenname'];
	
			$this->copyfailnum = 0;
			$this->copydir($copy_from.DIRECTORY_SEPARATOR.'bangcms'.DIRECTORY_SEPARATOR.'plugin', $copy_to_pcpath, $_GET['cover']);
	...
	public function config() {
		if(isset($_POST['dosubmit'])) {
			...
			if(!isset($_GET['module'])) {	
				...
				$plugin_admin_path = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$identification.DIRECTORY_SEPARATOR.'plugin_admin.class.php';
	...
	function set_hook_cache() {
		if($info = $this->db->select(array('disable'=>1),'*','','listorder DESC')) {
			foreach($info as $i) {
				$id = $i['identification'];
				$hook_file = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$id.DIRECTORY_SEPARATOR.'hook.class.php';
	 ...
	 */	
 	private function _sql_split($sql) {
		$database = bc_base::load_config('database');
