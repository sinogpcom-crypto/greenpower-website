...
class position extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('position_model');
		$this->db_data = bc_base::load_model('position_data_model');
		$this->db_content = bc_base::load_model('content_model');			
		$this->sites = bc_base::load_app_class('sites');
	}
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->sitemodel_db = bc_base::load_model('sitemodel_model');
	...
	public function public_item() {	
		if(isset($_POST['dosubmit'])) {
			...
			foreach ($pos_arr as $_k => $_v) {
				...
				$r['catid'] = $_v['catid'];
				$temp_data = string2array($_v['data']);
				$r['is_relation_model'] = false;
				if(!empty($temp_data['n_relation_field']) && !empty($temp_data['n_pid']) ) {
					$r['url'] = go($_v['catid'], $_v['id'], 0, $_v['modelid']);
					$r['is_relation_model'] = true;
					$r['n_relation_field'] = $temp_data['n_relation_field'];
					$r['n_pid'] = $temp_data['n_pid'];
				} else {
