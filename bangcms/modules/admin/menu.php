...
class menu extends admin {
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('menu_model');
	}
	...
	function add() {
		if(isset($_POST['dosubmit'])) {
			...
			//开发过程中用于自动创建语言包
			$file = BC_PATH.'languages'.DIRECTORY_SEPARATOR.'zh-cn'.DIRECTORY_SEPARATOR.'system_menu.lang.php';
			if(file_exists($file)) {
	...
	private function update_menu_models($id, $old_data, $new_data) {
		$models_config = bc_base::load_config('model_config');
