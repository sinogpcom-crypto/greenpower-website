...
class index extends admin {
	public function __construct() {
		...
		$this->panel_db = bc_base::load_model('admin_panel_model');
	}
	...
	public function init () {
		...
		$site = bc_base::load_app_class('sites');
	...
	public function login() {
		if(isset($_GET['dosubmit'])) {
			$_POST['username'] = decode1($_POST['username']);
			$_POST['password'] = decode1($_POST['password']);
			$_POST['code'] = decode1($_POST['code']);
			//不为口令卡验证
			if (!isset($_GET['card'])) {
			...
			//查看是否使用口令卡
			if (!isset($_GET['card']) && $r['card'] && bc_base::load_config('system', 'safe_card') == 1) {
				$_SESSION['card_username'] = $username;
				$_SESSION['card_password'] = $_POST['password'];
				header("location:?m=admin&c=index&a=public_card");
				exit;
			} elseif (isset($_GET['card']) && bc_base::load_config('system', 'safe_card') == 1 && $r['card']) {//对口令卡进行验证
			...
			if ($video_setting['sn'] && $video_setting['skey']) {
				$vmsapi = bc_base::load_app_class('ku6api', 'video');
	...
	public function public_card() {
		...
		if(!$r) showmessage(L('user_not_exist'),'?m=admin&c=index&a=login');
		if (isset($_GET['dosubmit'])) {
			bc_base::load_app_class('card', 'admin', 0);
	...
	public function public_main() {
		...
		bc_base::load_app_func('admin');
		define('BC_VERSION', bc_base::load_config('version','bc_version'));
		define('BC_RELEASE', bc_base::load_config('version','bc_release'));	
	...
	public function public_snda_status() {
		//引入盛大接口
		if(!strstr(bc_base::load_config('snda','snda_status'), '|')) {
			$this->site_db = bc_base::load_model('site_model');
			$uuid_arr = $this->site_db->get_one(array('siteid'=>1), 'uuid');
			$uuid = $uuid_arr['uuid'];
			$snda_check_url = "http://open.sdo.com/bangcms?cmsid=".$uuid."&sitedomain=".$_SERVER['SERVER_NAME'];
	...
	public function public_set_model() {
		...
		if (!$model) {
			...
			$models = bc_base::load_config('model_config');
		...
		$menudb = bc_base::load_model('menu_model');
