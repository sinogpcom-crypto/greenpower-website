...
class html {
	...
	public function __construct() {
		...
		$this->site = bc_base::load_app_class('sites', 'admin');
	...
	public function _create_content($contentid = 0) {
		...
		if ($arr_content['paginationtype']) {			//文章使用分页时
			if($arr_content['paginationtype']==1) {
				...
				$contentpage = bc_base::load_app_class('contentpage', 'content'); //调用自动分页类
		...
		if ($CONTENT_POS !== false) {
			...
			for ($i=1; $i<=$pagenumber; $i++) {
				...
				$file = BANGCMS_PATH.$file; //生成文件的路径
	...
	public function _index($specialid = 0, $pagesize = 20, $pages_num = 0) {
		...
		if ($siteid>1) {
			...
			$file = bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/index.html';
		} else {
			$file = bc_base::load_config('system', 'html_root').'/special/'.$filename.'/index.html';
		}
		if(!$ispage) {
		...
		if($ispage) {
			...
			for ($i=1; $i<=$pages_num; $i++) {
				...
				$file_root = BANGCMS_PATH.$file_root;
				ob_start();
				include template('special', $template);
	...
	public function create_list() {
		...
		if ($siteid>1) {
			$site_info = $this->site->get_by_id($siteid);
			$file = bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/index.html';
		} else {
			$file = bc_base::load_config('system', 'html_root').'/special/index.html';
		}
		$this->queue->add_queue('add', $file, $siteid);
		$file  = BANGCMS_PATH.$file;
		ob_start();
	...
	public function create_type($typeid = 0, $page = 1) {
		...
		extract($s_info);
		$site_info = $this->site->get_by_id($siteid);
		define('URLRULE', $site_info['domain'].substr(bc_base::load_config('system', 'html_root'), 1).'/special/{$specialdir}/{$typedir}/type-{$typeid}.html~'.$site_info['domain'].substr(bc_base::load_config('system', 'html_root'), 1).'/special/{$specialdir}/{$typedir}/type-{$typeid}-{$page}.html');
		...
		if ($siteid>1) {
			if ($page==1) $file = bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'.html';
			else $file = bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'-'.$page.'.html';
		} else {
			if ($page==1) $file = bc_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'.html';
			else $file = bc_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/type-'.$typeid.'-'.$page.'.html';
		}
		$this->queue->add_queue('add', $file, $siteid);
		$file = BANGCMS_PATH.$file;
		ob_start();
