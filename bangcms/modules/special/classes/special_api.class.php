...
class special_api {
	...
	public function __construct() {
		$this->db = bc_base::load_model('special_model'); //专题数据模型
		$this->type_db = bc_base::load_model('type_model'); //专题分类数据模型
		$this->c_db = bc_base::load_model('special_content_model'); //专题内容数据模型
		$this->data_db = bc_base::load_model('special_c_data_model'); 
	}
	...
	public function _update_type($specialid, $type, $a = 'add') {
		...
		foreach ($type as $k => $v) {
			...
			if ($a == 'add' && !$v['del']) {
				...
				if ($siteid>1) {
					$site = bc_base::load_app_class('sites', 'admin');
					...
					if($special_info['ishtml']) $url = addslashes($app_path.bc_base::load_config('system', 'html_root').'/special/'.$special_info['filename'].'/'.$v['typedir'].'/'.'type-'.$typeid.'.html');
				...
				if ((!isset($v['del']) || empty($v['del'])) && $v['typeid']) {
					...
					if ($siteid>1) {
						...
						if($special_info['ishtml']) $url = addslashes($app_path.bc_base::load_config('system', 'html_root').'/special/'.$special_info['filename'].'/'.$v['typedir'].'/'.'type-'.$v['typeid'].'.html');
	...
	public function _get_import_data($modelid = 0, $where = '', $page) {
		$c = bc_base::load_model('content_model');
		if(!$modelid) return '';
		$c->set_model($modelid);
	...
	public function _del_special($id = 0) {
		...
		if ($info['siteid']>1) {
			if ($info['ishtml']) {
				...
				$file = bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$info['filename'].'/index.html';
				if ($info['ispage']) {
					for ($i==1; $i>0; $i++) {
						if ($i>1) {
							$file = str_replace('.html', '-'.$i.'.html', $file);
						}
						if (!file_exists(BANGCMS_PATH.$file)) {
							break;
						} else {
							$queue->add_queue('del', $file, $info['siteid']);
							unlink(BANGCMS_PATH.$file);
						}
					}
				} else {
					$queue->add_queue('del', $file, $info['siteid']);
				...
				$queue->add_queue('del', bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$info['filename'].'/', $info['siteid']);
				dir_delete(bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$info['filename']);
			}
		} else {
			if ($info['ishtml']) {
				dir_delete(BANGCMS_PATH.bc_base::load_config('system', 'html_root').DIRECTORY_SEPARATOR.'special'.DIRECTORY_SEPARATOR.$info['filename']); //删除专题目录
			}
		}
		if(bc_base::load_config('system','attachment_stat')) {
			$keyid = 'special-'.$id;
	...
	private function delete_type($typeid = 0, $siteid = 0, $ishtml = 0) {
		...
		if ($ishtml) {
			...
			if ($siteid>1) {
				...
				$queue = bc_base::load_model('queue_model');
				for ($i = 1; $i>0; $i++) {
					if ($i==1) $file = str_replace($site_info['domain'], bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/', $info['url']);
					else $file = str_replace(array($site_info['domain'], '.html'), array(bc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/', '-'.$i.'.html'), $info['url']);
					if (!file_exists(BANGCMS_PATH.$file)) {
						break;
					} else {
						$queue->add_queue('del', $file, $siteid); //并加入到消息队列中，便以其他站点删除文件
						unlink(BANGCMS_PATH.$file);	//删除生成的静态文件
					}
				}
			} else {
				for ($i = 1; $i>0; $i++) {
	...
	public function _delete_content($cid = 0, $siteid = 0, $ishtml = 0) {
		...
		if ($info['isdata']) {
			if ($ishtml) {	
				bc_base::load_app_func('global', 'special');
				$siteid = $siteid ? intval($siteid) : get_siteid();
				if ($siteid>1) {
					$site = bc_base::load_app_class('sites', 'admin');
					$site_info = $site->get_by_id($siteid);
					...
					for ($i = 1; $i>0; $i++) {
						...
						if (!file_exists(BANGCMS_PATH.$file[1])) {
							break;
						} else {
							$queue->add_queue('del', $file[1], $siteid); //并加入到消息队列中，便以其他站点删除文件
							unlink(BANGCMS_PATH.$file[1]);	//删除生成的静态文件
						}
					}
				} else {
					for ($i = 1; $i>0; $i++) {
			...
			// 删除数据统计表数据
			$count = bc_base::load_model('hits_model');
