...
class special extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('special_model');
		$this->special_api = bc_base::load_app_class('special_api', 'special');
	}
		...
		if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
			...
			if ($id) {
				...
				if ($special['siteid']>1) {
					...
					$url = $special['ishtml'] ? APP_PATH.substr(bc_base::load_config('system', 'html_root'), 1).'/special/'.$special['filename'].'/' : APP_PATH.'index.php?m=special&c=index&id='.$id;
				}
				$this->db->update(array('url'=>$url), array('id'=>$id, 'siteid'=>$this->get_siteid()));
				
				//调用生成静态类
				if ($special['ishtml']) {
					$html = bc_base::load_app_class('html', 'special'); 
					$html->_index($id, 20, 5);
				}
				//更新附件状态
		...
		if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
			...
			if ($special['ishtml'] && $special['filename']) {
				...
				if ($siteid>1) {
					...
					$special['url'] = APP_PATH.substr(bc_base::load_config('system', 'html_root'), 1).'/special/'.$special['filename'].'/';
				}
	...
	public	function create_special_list() {
		$html = bc_base::load_app_class('html');
	...
	public function public_create_type() {
		...
		if (is_array($types) && !empty($types) || $pages) {
			...
			if (!$pages) {
				$c = bc_base::load_model('special_content_model');
			...
			$special_api = bc_base::load_app_class('special_api');
	...
	public function public_check_special() {
		...
		if(bc_base::load_config('system', 'charset')=='gbk') {
	...
	private function check($data, $a = 'add') {
		...
		if ($data['voteid']) {
			if (strpos($data['voteid'], '|')===false) {
				$vote_db = bc_base::load_model('vote_subject_model');
