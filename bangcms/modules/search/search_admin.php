		...
		if(isset($_GET['dosubmit'])) {
			...
			foreach ($types as $_k=>$_v) {
				if($key==$_k) {
					...
					if($_v['modelid']) {
						if ($_v['typedir']!=='yp') {
							$search_api = bc_base::load_app_class('search_api','content');
						} else {
							$search_api = bc_base::load_app_class('search_api',$_v['typedir']);
						}
						if(!isset($_GET['total'])) {
						...
						$search_api = bc_base::load_app_class('search_api',$module);
						if(!isset($_GET['total'])) {
