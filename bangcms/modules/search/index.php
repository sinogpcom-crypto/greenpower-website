...
class index {
	function __construct() {
		$this->db = bc_base::load_model('search_model');
	...
	public function init() {
		...
		if(isset($_GET['q'])) {
			...
			if($setting['sphinxenable']) {
				$sphinx = bc_base::load_app_class('search_interface', '', 0);
				...
				bc_base::load_sys_class('segment', '', 0);
				$segment = new segment();
				//分词结果
				//$segment_q = $segment->get_keyword($segment->split_result($q));
				//如果分词结果为空
				/*if(!empty($segment_q)) {
					$sql = "`siteid`= '$siteid' AND `typeid` = '$typeid' $sql_time AND MATCH (`data`) AGAINST ('$segment_q' IN BOOLEAN MODE)";REGEXP 
				} else {*/
					//$sql = "`siteid`= '$siteid' AND `typeid` = '$typeid' $sql_time AND `data` like '%$q%'";
					/*$sql = "`siteid`= '$siteid' AND `typeid` IN (1,88,89) $sql_time AND `data` REGEXP  '$q'";*/
			...
			if($setting['relationenble']) {
				...
				$this->keyword_db = bc_base::load_model('search_keyword_model');
                    ...
                    if($modelid) {
                        ...
                        */
                        if(empty($this->content_db->model_tablename)) {
                            $this->content_db = bc_base::load_model('yp_content_model');
                            $this->content_db->set_model($modelid);
