...
class ku6api {
	...
	public function __construct($ku6api_sn = '', $ku6api_skey = '') {
		...
		$this->ku6api_url = bc_base::load_config('ku6server', 'api_url');
		$this->ku6api_api = bc_base::load_config('ku6server', 'api');
		$this->http = bc_base::load_sys_class('http');
		$this->xxtea = bc_base::load_app_class('xxtea', 'video');
	...
	public function update_video_status_from_vms() {
		if (is_array($_GET) && !empty($_GET)) {
			...
			$db = bc_base::load_model('video_store_model');
	...
	public function update_vms_member($data = array()) {
		...
		$api_url = bc_base::load_config('sub_config','member_add_dir').'MemberUpgrade.php';
