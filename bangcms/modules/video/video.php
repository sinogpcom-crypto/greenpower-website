...
bc_base::load_sys_class('form', 0, 0);
...
class video extends admin {
	...
	public function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('video_store_model');
		$this->module_db = bc_base::load_model('module_model');
		$this->userid = $_SESSION['userid'];
		bc_base::load_app_class('ku6api', 'video', 0);
		bc_base::load_app_class('v', 'video', 0);
	...
	public function public_update_content() {
		...
		$url = bc_base::load_app_class('url', 'content');
	...
	public function public_get_pos () {
		...
		$category = bc_base::load_model('category_model');
	...
	*/
	public function get_category(){
		...
		$sitemodel_field = bc_base::load_model('sitemodel_field_model');
		...
		$cat_db = bc_base::load_model('category_model');
		if ($models) {
			...
			if (is_array($result)) { 
				...
				$data = $return_data = $categorys = array(); 
				$tree = bc_base::load_sys_class('tree');//factory::load_class('tree', 'utils');
	...
	public function public_check_status() {
		...
		$status_arr = bc_base::load_config('ku6status_config');
		if ($return['ku6status'] != $r['status']) {
			...
			if ($return['ku6status']==21) {
				...
				if (is_array($result) && !empty($result)) {
					...
					$html = bc_base::load_app_class('html', 'content');
					$content_db = bc_base::load_model('content_model');
					$content_check_db = bc_base::load_model('content_check_model');
					$url = bc_base::load_app_class('url', 'content');
					foreach ($result as $rs) {
						$modelid = intval($rs['modelid']);
						$contentid = intval($rs['contentid']);
