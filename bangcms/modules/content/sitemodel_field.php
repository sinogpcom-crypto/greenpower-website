...
class sitemodel_field extends admin {
	...
	function __construct() {
		...
		$this->model_db = bc_base::load_model('sitemodel_model');
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if ($_POST['issystem']) {
				$_POST['info']['issystem'] = $_POST['issystem'];
			}
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			//获取模型、及关联模型数据
			if ($formtype=='onetomany') {
				$model_cache = getcache('model','commons');
				$this->cache_field($setting['relation_model']);
				$model_fields = $this->db->select(array('modelid'=>$setting['relation_model']),'*',100,'listorder ASC');
				$select_fields = array();
				foreach ( $model_fields as $v ) {
					if ($v['issystem']=='1' && $v['formtype']=='number' && $v['field']!='listorder') {
						$select_fields[] = $v;
					}
				}
			}
	...
	public function get_model_field(){
		...
		foreach ( $datas as $v ) {
			if ($v['issystem']=='1' && $v['formtype']=='number' && $v['field']!='listorder') {
				$str .= '<option value="'.$v['field'].'">'.$v['name'].'</option>';
			}
		}
		echo $str;
		return true;
