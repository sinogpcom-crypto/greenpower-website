        ...
        <tr>
			...
			<?php
				...
				foreach ($onetomany_fields as $onetomany_field2) {
					$onetomany_url = "?m=detail&c=content&a=init&catid={$catid}&parent_model_id={$onetomany_field2['parent_model_id']}&relation_model={$onetomany_field2['relation_model']}&relation_field={$onetomany_field2['relation_field']}&pid={$r['id']}&pc_hash={$_SESSION['pc_hash']}";
					echo '<td align="center"><a href="'.$onetomany_url.'">'.$onetomany_field2['name'].'&gt;&gt;<a></td>';
				}
			?>
...
function push() {
	...
	window.top.art.dialog({title:'<?php echo L('push');?>：',id:'push',iframe:'?m=content&c=push&a=init&module=content&classname=push_api&action=category_list&order=3&tpl=push_to_category&catid=<?php echo $catid?>&modelid=<?php echo $modelid?>&id='+id,width:'800',height:'500'}, function(){var d = window.top.art.dialog({id:'push'}).data.iframe;// 使用内置接口获取iframe对象
