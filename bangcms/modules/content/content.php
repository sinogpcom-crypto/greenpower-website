...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			if ($listinfo_enough) {
				...
				$datas = $this->db->listinfoall($where,'listorder desc,m.inputtime desc',$_GET['page']);//取出包含附属表的全部信息
			}
			$pages = $this->db->pages;
		...
		if($workflowid) {
			...
			//更改内容状态
				...
				if ($status==0) {
					...
					$this->admin = bc_base::load_model('admin_model');
					$f_info = $this->db->get_one(array('catid'=>$catid));
					$now_admin = $this->admin->get_one(array('username'=>$admin_username));
					$mail_admin = $this->admin->get_one(array('username'=>$f_info['username']));
					$mysite=siteinfo($this->siteid);
					$zhuti = '退稿邮件';
					
					
	                $Fields = '<p>尊敬的管理员：</p>';
	                $Fields .= '<p style="text-indent: 2em;">您的提交的文章已被管理员 '.$now_admin['realname'].' 退回，请及时查看。</p>';
	                $Fields .= '<p>所属栏目：'.$this->categorys[$catid]['catname'].'；<br/>';
	                $Fields .= '退稿文章：'.$f_info['title'].'；<br/>';
	                if(isset($_POST['reject_c']) && $_POST['reject_c'] != L('reject_msg')) {
						$Fields .= '退稿理由：'.$_POST['reject_c'].'；</p>';
					} elseif(isset($_GET['reject_c']) && $_GET['reject_c'] != L('reject_msg')) {
						$Fields .= '退稿理由：'.$_GET['reject_c'].'；</p>';
					}
	                
	                $Fields .= '<p style="text-align: right;">'.$mysite['name'].'</p>';
	                $Fields .= '<p style="text-align: right;">'.date('Y-m-d H:i:m').'</p>';
	               
	               
					sendmail($mail_admin['email'], $zhuti, $Fields);
					//p($Fields);exit;
			...
			$tree->init($categorys);
				switch($from) {
					...
					default:
						...
						//$strs2 = "<span class='folder'>\$catname</span><a class='self' href='?m=content&c=content&a=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right' onclick='open_list(this)'>【 \$catname 】内容</a>";//
	...
	public function public_getjson_ids() {
		$onmodelid = intval($_GET['onmodelid']);
		$modelid = intval($_GET['modelid']);
		$id = intval($_GET['id']);
		$this->db->set_model($onmodelid);
