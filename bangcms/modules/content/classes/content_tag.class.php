...
class content_tag {
	...
	public function __construct() {
		...
		$this->position = bc_base::load_model('position_data_model');
	}
	/**
	 * 初始化模型
	 * @param $catid
	...
	public function lists($data) {
		...
		if(isset($data['self']) && intval($data['self']) == 1){
	...
	public function position($data) {
		...
		if(!empty($pos_arr)) {
			foreach ($pos_arr as $info) {
				...
				if( !empty($array[$key]['n_relation_field']) && !empty($array[$key]['n_pid']) ) {
					$array[$key]['url'] = go($info['catid'],$info['id'], 0, $info['modelid']);
				} else {
