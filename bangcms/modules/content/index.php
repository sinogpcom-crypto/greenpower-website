...
class index {
	...
	public function show() {
		...
		$inputtimes = strtotime($inputtime);
		//上一页
		//$previous_page = $this->db->get_one("`catid` = '$catid' AND `id`<'$id' AND `status`=99",'*','id DESC');
		$sql_pre = "select * from `".$this->db->table_name."`
						where 
						`catid` = '$catid' AND `status`=99 
						AND
						(
						case 
						when `listorder`='$listorder'  then `inputtime`>'$inputtimes'
						when `listorder`>'$listorder'  then `listorder`>'$listorder'
						else 0 end
						)
						order by listorder ASC, inputtime ASC
						limit 1";
		$previous_page =$this->db->query($sql_pre);
		$previous_page =$this->db->fetch_array();
		$previous_page = $previous_page[0];


		//下一页
		//$next_page = $this->db->get_one("`catid`= '$catid' AND `id`>'$id' AND `status`=99");
		$sql_next = "select * from `".$this->db->table_name."`
						where 
						`catid` = '$catid' AND `status`=99 
						AND
						(
						...
						order by listorder DESC, inputtime DESC
						limit 1";
		$next_page = $this->db->query($sql_next);
		$next_page =$this->db->fetch_array();
	...
	protected function _category_priv($catid) {
		...
		$this->category_priv_db = bc_base::load_model('category_priv_model');
	 ...
	 // 	$id = intval($_GET['id']);
	 // 	$this->xiazai = bc_base::load_model('xiazai_model');
		// $result = $this->xiazai->select(array('catid'=>$catid,'is_admin'=>0,'action'=>'visit'));
	 	//$fileName = $_GET['filename']; //?õ?????
	 	$fileName = "/uploadfile/2021/0916/20210916085913838.docx"; //

	 	//file_down($fileName);exit;

  		//chmod($fileName,0755);
	 	// p(remote_filesize($fileName));
	 	// p(filesize($fileName));
	 	// p(filetype($fileName));
	 	// p(filemtime($fileName));exit;
	 	...
	 	// if (file_exists($fileName)) {
			...
			header ( 'Content-Length: ' . filesize ( $fileName )); 
			ob_clean (); 
			ob_end_clean();//https://blog.csdn.net/chen110710/article/details/107028465/
			flush (); 
			readfile ( $fileName ); 
			exit;
	 	// }else{
