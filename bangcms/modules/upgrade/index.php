...
class index extends admin {
	private $_filearr = array('api', 'bangcms', 'statics', '');
	//md5验证地址
	private $_upgrademd5 = 'http://www.zhengbang.com.cn/upgrademd5/';
	//补丁地址
	private $_patchurl = 'http://download.zhengbang.com.cn/v9/9.0/patch/';
	...
	public function init() {
		...
		//获取当前版本
		$current_version = bc_base::load_config('version');
		...
		if(!empty($_GET['s'])) {
			...
			foreach($pathlist as $k=>$v) {
					...
					foreach ($file_list as $fk=>$fv) {
						if(in_array(strtolower(substr($fv, -3, 3)), array('php', 'sql'))) {
							if (strtolower(substr($file_list[$fk], -3, 3)) == 'sql' && $data = file_get_contents($file_list[$fk])) {
								...
								if (!$db = bc_base::load_model($model_name.'_model')) {
								...
								if (strtolower(basename($file_list[$fk])) == 'system_menu.lang.php' && file_exists($file_list[$fk])) {
									...
									$menu_lan_file = BC_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system_menu.lang.php';
				...
				if(file_exists($configpath)) {
					...
					$version_arr = array('bc_version'=>$config_arr['to_version'], 'bc_release'=>$config_arr['to_release']);
					//版本文件地址
	...
	public function checkfile() {
		if(!empty($_GET['do'])) {
			...
			//读取bangcms接口
			$current_version = bc_base::load_config('version');
			$bangcms_md5 = @file_get_contents($this->_upgrademd5.$current_version['pc_release'].'_'.CHARSET.".php");
			$bangcms_md5_arr = json_decode($bangcms_md5, 1);
			//计算数组差集
			$diff = array_diff($bangcms_md5_arr, $this->md5_arr);

			//丢失文件列表
			$lostfile = array();
			foreach($bangcms_md5_arr as $k=>$v) {
			...
			//未知文件列表
			$unknowfile = array_diff(array_keys($this->md5_arr), array_keys($bangcms_md5_arr));
