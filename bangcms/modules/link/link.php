...
class link extends admin {
	function __construct() {
		...
		$this->db = bc_base::load_model('link_model');
		$this->db2 = bc_base::load_model('type_model');
	}
	...
	public function public_name() {
		$link_title = isset($_GET['link_name']) && trim($_GET['link_name']) ? (bc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['link_name'])) : trim($_GET['link_name'])) : exit('0');
	...
	public function public_check_name() {
		$type_name = isset($_GET['type_name']) && trim($_GET['type_name']) ? (bc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['type_name'])) : trim($_GET['type_name'])) : exit('0');
 	...
 	public function add() {
 		if(isset($_POST['dosubmit'])) {
			...
			if(!$linkid) return FALSE; 
	 		...
	 		//更新附件状态
			if(bc_base::load_config('system','attachment_stat') & $_POST['link']['logo']) {
