...
class block_admin extends admin {
	...
	public function __construct() {
		$this->db = bc_base::load_model('block_model');
		$this->priv_db = bc_base::load_model('block_priv_model');
		$this->history_db = bc_base::load_model('block_history_model');
	...
	public function block_update() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($data['type'] == 2) {
				...
				if ($template) {
					$block = bc_base::load_app_class('block_tag');
	...
	public function public_view() {
		...
		if ($data['type'] == 1) {
			...
			$tpl = bc_base::load_sys_class('template_cache');
