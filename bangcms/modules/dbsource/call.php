...
class call  {
	...
	public function __construct() {
		$this->db = bc_base::load_model('datacall_model');
	}
		...
		if ($data = $this->db->get_one(array('id'=>$id))) {
			if (!$str = tpl_cache('dbsource_'.$id,$data['cache'])) {
				if ($data['type'] == 1) { //自定义SQL调用
					$get_db = bc_base::load_model("get_model");
					...
					$filepath = BC_PATH.'modules'.DIRECTORY_SEPARATOR.$data['module'].DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$data['module'].'_tag.class.php';
					if (file_exists($filepath)) {
						$pc_tag = bc_base::load_app_class($data['module'].'_tag', $data['module']); 
	...
	private function _format($id, $data, $type) {
		switch($type) {
			...
			case '2'://xml
				$xml = bc_base::load_sys_class('xml');
