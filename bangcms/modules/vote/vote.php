...
class vote extends admin {
	...
	function __construct() {
		...
		$this->db = bc_base::load_model('vote_subject_model');
		$this->db2 = bc_base::load_model('vote_option_model');
	}
	...
	public function public_name() {
		$subject_title = isset($_GET['subject_title']) && trim($_GET['subject_title']) ? (bc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['subject_title'])) : trim($_GET['subject_title'])) : exit('0');
	...
	public function statistics() {
			...
			$show_validator = $show_scroll = $show_header = true;
 			//获取投票信息
			$sdb = bc_base::load_model('vote_data_model'); //加载投票统计的数据模型
	...
	public function statistics_userlist() {
			...
			$sdb = bc_base::load_model('vote_data_model'); //调用统计的数据模型
 			$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
