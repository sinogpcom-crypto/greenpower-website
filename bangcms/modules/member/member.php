	...
	*/
		...
		foreach($memberlist_arr as $k=>$v) {
			...
			if ($this->sso_open){
			    $memberlist[$k]['avatar'] = get_memberavatar($v['phpssouid']);
 			    ...
 			    $memberlist[$k]['avatar'] = get_memberavatar($v['userid'],TRUE);
 			}
		}
	...
	function add() {
		...
		if(isset($_POST['dosubmit'])) {
            ...
            if (isset($info['pwdconfirm'])) unset($info['pwdconfirm']);
		...
		if(isset($_POST['dosubmit'])) { //处理提交
			...
			if ($this->sso_open){
			    $memberinfo['avatar'] = get_memberavatar($memberinfo['phpssouid'], '', 90);
 			    ...
 			    $memberinfo['avatar'] = get_memberavatar($memberinfo['userid'], true, 90);
 			}
	...
	function delete() {
		...
		if(!empty($phpssouidarr) && $this->sso_open) {
	...
	function memberinfo() {
		...
		if ($this->sso_open){
		    ...
		    $memberinfo['avatar'] = get_memberavatar($memberinfo['userid'], true, 90);
		    $phpsso_api_url = $this->client->get_api_url();
		}
		$grouplist = getcache('grouplist');
		//会员模型缓存
