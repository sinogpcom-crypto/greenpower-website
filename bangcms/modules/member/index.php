...
/**
 * 会员前台管理中心、账号管理、收藏操作类
 * $Id: index.php 177 2014-12-01 07:17:39Z jiaokun $
 */

defined('IN_BANGCMS') or exit('No permission resources.');
bc_base::load_app_class('foreground');
...
class index extends foreground {
	...
	function __construct() {
		...
		$this->sso_open = bc_base::load_config('system', 'phpsso');
	}
    /**
     * 会员中心-首页
     */
	public function init() {
		...
		if ($this->sso_open){
		    ...
		    $phpsso_api_url = $this->_init_client();
		    $avatar = $this->client->get_avatar($this->memberinfo['userid']);
		}
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($member_setting[mobile_checktype]=='1'){
				...
				if($mobile_verify=='') showmessage('请提供正确的手机验证码！', HTTP_REFERER);
 				$sms_report_db = bc_base::load_model('sms_report_model');
			...
			if($member_setting['enablemailcheck']) {	//是否需要邮件验证
				...
				$this->verify_db = bc_base::load_model('member_verify_model');
			...
			//传入phpsso为明文密码，加密后存入bangcms
			$password = $userinfo['password'];
			$userinfo['password'] = password($userinfo['password'], $userinfo['encrypt']);
			$userid = $this->db->insert($userinfo, 1); //保存会员数据
			...
			if($userid > 0) {
			    ...
			    if($userinfo['groupid'] == 7) { //邮件认证组
			        param::set_cookie('_username', $userinfo['username'], $cookietime);
			        param::set_cookie('email', $userinfo['email'], $cookietime);
			    } else {
			        $bangcms_auth_key = md5(bc_base::load_config('system', 'auth_key').$this->http_user_agent);
			        $bangcms_auth = sys_auth($userid."\t".$userinfo['password'], 'ENCODE', $bangcms_auth_key);
			        	
			        param::set_cookie('auth', $bangcms_auth, $cookietime);
			...
			if($member_setting['enablemailcheck']) {
			    ...
			    $bangcms_auth_key = md5(bc_base::load_config('system', 'auth_key'));
			    $code = sys_auth($userid.'|'.SYS_TIME, 'ENCODE', $bangcms_auth_key);
			...
			if(!empty($_GET['verify'])) {
				...
				$bangcms_auth_key = md5(bc_base::load_config('system', 'auth_key'));
				$code_res = sys_auth($code, 'DECODE', $bangcms_auth_key);
	...
	public function check_email($mail){
	    ...
	    $re = $this->db->get_one(array('email'=>$mail));
	    return !empty($re) ? -1 : 1;
	}
	/**
	 * 测试邮件配置
	...
	public function send_newmail() {
		...
		if ($this->sso_open){ //单点登录
		    ...
		    //非单点登录
		    $status = $this->check_email($newemail);
		}
		if($status=='-5'){//邮箱禁止注册
    		exit('-1');
		}
	...
	public function account_manage_avatar() {
		...
		if ($this->sso_open){
		    //初始化phpsso
		    $phpsso_api_url = $this->_init_phpsso();
		    $ps_auth_key = bc_base::load_config('system', 'phpsso_auth_key');
		    ...
		    $phpsso_api_url = $this->_init_client();
		    $upurl = base64_encode($phpsso_api_url.'/index.php?m=member&c=index&a=uploadavatar&uid='.$this->memberinfo['userid']);
		    $avatar = $this->client->get_avatar($this->memberinfo['userid']);
		}
    ...
    public function uploadavatar(){
        //根据用户id创建文件夹
        if(isset($_REQUEST['uid']) && isset($GLOBALS['HTTP_RAW_POST_DATA'])) {
            $uid = $_REQUEST['uid'];
            $avatar_data = $GLOBALS['HTTP_RAW_POST_DATA'];
        } else {
            exit('0');
        }
        $dir1 = ceil($uid / 10000);
        ...
        //检查avatar字段是否存在
        $this->_check_avatar_field_exists();
        
        $this->db->update(array('avatar'=>1), array('userid'=>$uid));
        exit('1');
    }
    /*
     * 非单点登录：检查avatar字段是否存在，如果不存在则建立之
     */
    private function _check_avatar_field_exists(){
        if (!$this->db->field_exists('avatar')){
            $sql = "ALTER TABLE `".$this->db->get_table_name()."` ADD `avatar` TINYINT UNSIGNED NOT NULL DEFAULT '0' COMMENT '1=有头像'";
            $this->db->query($sql);
        }
    }
    /**
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if ($this->sso_open){
				...
				//密码错误剩余重试次数
				$this->times_db = bc_base::load_model('times_model');
			...
			$bangcms_auth_key = md5(bc_base::load_config('system', 'auth_key').$this->http_user_agent);
			$bangcms_auth = sys_auth($userid."\t".$password, 'ENCODE', $bangcms_auth_key);
	...
	public function favorite() {
		$this->favorite_db = bc_base::load_model('favorite_model');
		$memberinfo = $this->memberinfo;
	...
	public function friend() {
		$memberinfo = $this->memberinfo;
		$this->friend_db = bc_base::load_model('friend_model');
	...
	public function change_credit() {
		...
		if(isset($_POST['dosubmit'])) { //此分支只有打开单点登录时才会执行
			//本系统积分兑换数
			...
			$credit_list = bc_base::load_config('credit');
	...
	public function mini() {
		...
		$snda_enable = bc_base::load_config('system', 'snda_enable');
	...
	private function _init_phpsso() {
		bc_base::load_app_class('client', '', 0);
	...
	private function _init_client() {
	    bc_base::load_app_class('client_member', '', 0);
	    $this->client = new client_member();
	    return $this->client->get_api_url();
	}
	...
	public function public_checkname_ajax() {
	    ...
	    if ($this->sso_open){
	        ...
	        $status = $this->client->ps_checkname($username);
 	        ...
 	        $this->_init_client();
 	        $status = $this->client->check_name($username);
 	    }
	...
	public function public_checkemail_ajax() {
		...
		if ($this->sso_open){
		    ...
		    $this->_init_client();
		    $status = $this->client->check_email($email);
		}
		if($status == -5) {	//禁止注册
			exit('0');
		} elseif($status == -1) {	//已存在，但是修改用户的时候需要判断邮箱是否是当前用户的
		    if ($this->sso_open){
		        ...
		        if(isset($_GET['uid']) && !empty($_GET['uid'])) {	//后台修改验证：用户传入uid
		            //可直接修改
		            exit('1');
		        }else {
		            exit('0');
	...
	public function public_sina_login() {
		define('WB_AKEY', bc_base::load_config('system', 'sina_akey'));
		define('WB_SKEY', bc_base::load_config('system', 'sina_skey'));
		define('WEB_CALLBACK', APP_PATH.'index.php?m=member&c=index&a=public_sina_login&callback=1');
		bc_base::load_app_class('saetv2.ex', '' ,0);
		$this->_session_start();
	...
	public function public_snda_login() {
		define('SNDA_AKEY', bc_base::load_config('system', 'snda_akey'));
		define('SNDA_SKEY', bc_base::load_config('system', 'snda_skey'));
		define('SNDA_CALLBACK', urlencode(APP_PATH.'index.php?m=member&c=index&a=public_snda_login&callback=1'));
		
		bc_base::load_app_class('OauthSDK', '' ,0);
		$this->_session_start();		
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
	...
	public function public_qq_loginnew(){
                $appid = bc_base::load_config('system', 'qq_appid');
                $appkey = bc_base::load_config('system', 'qq_appkey');
                $callback = bc_base::load_config('system', 'qq_callback');
                bc_base::load_app_class('qqapi','',0);
	...
	public function public_qq_login() {
		define('QQ_AKEY', bc_base::load_config('system', 'qq_akey'));
		define('QQ_SKEY', bc_base::load_config('system', 'qq_skey'));
		bc_base::load_app_class('qqoauth', '' ,0);
		$this->_session_start();
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$code = sys_auth($memberinfo['userid']."\t".SYS_TIME, 'ENCODE', $bangcms_auth_key);
			...
			$code = sys_auth($_GET['code'], 'DECODE', $bangcms_auth_key);
			$code = explode("\t", $code);
	...
	/**
	*通过手机修改密码
	*方式：用户发送HHPWD afei985#821008 至 1065788 ，BangCMS进行转发到网站运营者指定的回调地址，在回调地址程序进行密码修改等操作,处理成功时给用户发条短信确认。
	*bangcms 以POST方式传递相关数据到回调程序中
	...
	*/
	public function public_changepwd_bymobile(){
		...
		//判断是否BangCMS请求的接口
		bc_base::load_app_func('global','sms');
		bc_base::load_app_class('smsapi', 'sms', 0);
