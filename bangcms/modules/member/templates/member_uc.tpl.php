...
<div class="pad-lr-10">
<form action="?m=member&c=member_setting&a=uc" method="post" name="form_messagequeue_manage">
...
<!--
	function mysql_test() {
	$.get(
			'?m=member&c=member_setting&a=myqsl_test', 
			{host:$('#uc_dbhost').val(),username:$('#uc_dbuser').val(), password:$('#uc_dbpw').val(), pc_hash:'<?php echo $_GET['pc_hash']?>'},function(data){if(data==1){alert('<?php echo L('connect_success')?>')}else{alert('<?php echo L('connect_failed')?>')}});
}
//-->
</script>
</body>
