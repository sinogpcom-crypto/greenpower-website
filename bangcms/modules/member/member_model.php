...
//模型原型存储路径
define('MODEL_PATH',BC_PATH.'modules'.DIRECTORY_SEPARATOR.'member'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
//模型缓存路径
...
class member_model extends admin {
	...
	function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if($modelid) {
				...
				//更新模型缓存
				bc_base::load_app_class('member_cache','','');
	...
	function delete() {
		...
		if ($this->db->delete($where)) {
			...
			//修改用户模型组为普通会员
			$this->member_db = bc_base::load_model('members_model');
