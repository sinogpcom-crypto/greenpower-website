...
<form action="?m=job&c=job&a=edit&jobid=<?php echo $jobid; ?>" method="post" name="myform" id="myform">
<table cellpadding="2" cellspacing="1" class="table_form" width="100%">
	
	<tr>
		<th width="100"><?php echo L('zhiwei')?>：</th>
		<td><input type="text" name="job[zhiwei]" value="<?php echo $zhiwei?>" id="zhiwei" size="30" class="input-text"></td>
	</tr>
	
	<tr>
		<th width="100">发布时间：</th>
		<td><?php echo form::date('job[enddate]', $enddate, 1);?></td>
	</tr>
		<tr>
		<th width="100"><?php echo L('diqu')?>：</th>
		<td><input type="text" name="job[diqu]" value="<?php echo $diqu?>" id="diqu" size="30" class="input-text"></td>
	</tr>
	
		<tr>
		<th width="100">招聘部门：</th>
		<td><input type="text" name="job[bumen]" value="<?php echo $bumen?>" id="bumen" size="30" class="input-text"></td>
	</tr>
	
		<tr>
		<th width="100">职位详情：</th>
		<td><textarea name="job[content]" id="content"><?php echo $content?></textarea><?php echo form::editor('content');?></td>
	</tr>
<!--	
	<tr>
		<th width="100"><?php echo L('renshu')?>：</th>
		<td><input type="text" name="job[renshu]" value="<?php echo $renshu?>" id="renshu" size="30" class="input-text"></td>
	</tr>
-->
	...
	<tr>
		<th width="100"><?php echo L('content')?>：</th>
		<td><textarea name="job[content]" id="content"><?php echo $content?></textarea><?php echo form::editor('content');?></td>
	</tr>
	
	<tr>
		<th width="100"><?php echo L('lianxiren')?>：</th>
		<td><input type="text" name="job[lianxiren]" value="<?php echo $lianxiren?>" id="lianxiren" size="30" class="input-text"></td>
	</tr>
	
	<tr>
		<th width="100"><?php echo L('tel')?>：</th>
		<td><input type="text" name="job[tel]" value="<?php echo $tel?>" id="tel" size="30" class="input-text"></td>
	</tr>
	
	<tr>
		<th width="100"><?php echo L('email')?>：</th>
		<td><input type="text" name="job[email]" value="<?php echo $email?>" id="email" size="30" class="input-text"></td>
	</tr>
	
	<tr>
		<th width="100"><?php echo L('address')?>：</th>
		<td><input type="text" name="job[address]" value="<?php echo $address?>" id="address" size="30" class="input-text"></td>
	</tr>
-->
	<tr>
		<th><?php echo L('passed')?>：</th>
		<td>
			<input name="job[passed]" type="radio" value="1" <?php if($passed==1){echo "checked";}?>>&nbsp;<?php echo L('yes')?>&nbsp;&nbsp;
			<input name="job[passed]" type="radio" value="0" <?php if($passed==0){echo "checked";}?>>&nbsp;<?php echo L('no')?>
		</td>
	</tr>

	<tr>
		<th></th>
		<td><input type="hidden" name="forward" value="?m=job&c=job&a=edit&jobid=<?php echo $jobid; ?>"> <input
...
$(function(){
	...
	$("#zhiwei").formValidator({
		...
		type:"get",
		url:"",
		data:"m=job&c=job&a=public_check_zhiwei&jobid=<?php echo $_GET['jobid']?>",
		datatype:"html",
		cached:false,
		async:'true',
	...
	$("#enddate").formValidator({
		onshow:"<?php echo L("input").L('enddate')?>",
		onfocus:"<?php echo L("input").L('enddate')?>",
		oncorrect:"<?php echo L('correct')?>"
	});
