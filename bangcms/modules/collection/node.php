...
class node extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = bc_base::load_model('collection_node_model');
	...
	public function del() {
		if (isset($_POST['dosubmit'])) {
			...
			$content_db = bc_base::load_model('collection_content_model');
	...
	public function public_test() {
		$nodeid = isset($_GET['nodeid']) ? intval($_GET['nodeid']) : showmessage(L('illegal_parameters'), HTTP_REFERER);
		bc_base::load_app_class('collection', '', 0);
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
	...
	public function public_name() {
		$name = isset($_GET['name']) && trim($_GET['name']) ? (bc_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['name'])) : trim($_GET['name'])) : exit('0');
	...
	public function col_url_list() {
		...
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
			...
			if ($total_page > 0) {
				...
				$history_db = bc_base::load_model('collection_history_model');
				$content_db = bc_base::load_model('collection_content_model');
				$total = count($url);
				$re = 0;
	...
	public function import() {
		...
		$program_db = bc_base::load_model('collection_program_model');
	...
	public function content_del() {
		...
		if (is_array($id)) {
			$collection_content_db = bc_base::load_model('collection_content_model');
			$history_db = bc_base::load_model('collection_history_model');
			$del_array = $id;
			$ids = implode('\',\'', $id);
			if ($history) {
	...
	public function import_content() {
		...
		$attach_status = false;
		if(bc_base::load_config('system','attachment_stat')) {
			$attachment_db = bc_base::load_model('attachment_model');
			$att_index_db = bc_base::load_model('attachment_index_model');
			$attach_status = true;
		}
