...
class index extends admin {
	...
	public function public_file_count() {
		...
		foreach ($scan['dir'] as $v) {
			if (is_dir($v)) {
				...
				$list = array_merge($list, array(str_replace(BANGCMS_PATH, '', $v)=>md5_file($v)));
			}
		}
	...
	public function public_file_func() {
		...
		if (isset($scan['func']) && !empty($scan['func'])) {
			foreach ($file_list as $key=>$val) {
				$html = file_get_contents(BANGCMS_PATH.$key);
	...
	public function public_file_code() {
		...
		if (isset($scan['code']) && !empty($scan['code'])) {
			foreach ($file_list as $key=>$val) {
	            ...
	            if(strtolower(substr($key, -4)) == '.php' && function_exists('zend_loader_file_encoded') && zend_loader_file_encoded(BANGCMS_PATH.$key)) {
	...
	public function view() {
		...
		if (!file_exists(BANGCMS_PATH.$url)) {
			showmessage(L('file_not_exists'));
		}
		$html = file_get_contents(BANGCMS_PATH.$url);
	...
	public function md5_creat() {
		...
		switch ($pro) {
			case '1'://统计文件
				...
				ob_flush();
				ob_clean();
				$list = scan_file_lists(BANGCMS_PATH, 1, 'php', 0, 1);
