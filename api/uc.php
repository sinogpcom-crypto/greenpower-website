<?php
error_reporting(0);
define('BANGCMS_PATH', dirname(__FILE__).'/../');
if(!defined('IN_BANGCMS')) include BANGCMS_PATH.'/bangcms/base.php';

define('UC_KEY', bc_base::load_config('system', 'uc_key'));
define('UCUSE', bc_base::load_config('system', 'ucuse'));
...
if(in_array($get['action'], array('test', 'deleteuser', 'renameuser', 'gettag', 'synlogin', 'synlogout', 'updatepw', 'updatebadwords', 'updatehosts', 'updateapps', 'updateclient', 'updatecredit', 'getcreditsettings', 'updatecreditsettings'))) {
	$uc_note = new uc_note();
	header('Content-type: text/html; charset='.bc_base::load_config('system', 'charset'));
...
class uc_note {
	...
	function __construct() {
		$this->member_db = bc_base::load_model('member_model');
		bc_base::load_sys_class('uc_model', 'model', 0);
	...
	public function deleteuser($get,$post) {
		//bc_base::load_app_func('global', 'admin');
		//bc_base::load_app_class('messagequeue', 'admin' , 0);
	...
	public function updatepw($get,$post) {
		...
		if ($r) {
		    ...
		    $this->member_db->update(array('password'=>$r['password'], 'encrypt'=>$r['salt']), array('username'=>$username));
		}
		return API_RETURN_SUCCEED;
	}
	...
	public function synlogin($get,$post) {
		...
		if ($data = $this->member_db->get_one(array('ucuserid'=>$uid))) {//当用户存在时，获取用户的登陆信息
			...
			$this->member_db->update(array('lastip'=>$r['lastloginip'], 'lastdate'=>$r['lastlogindate']), array('userid'=>$data['userid']));
		} else { //当用户不存在是注册新用户
			...
			$datas = $data = array('username'=>$r['username'], 'password'=>$r['password'], 'encrypt'=>$r['salt'], 'email'=>$r['email'], 'regip'=>$r['regip'], 'regdate'=>$r['regdate'],  'lastdate'=>$r['lastlogindate'], 'from'=>'ucenter');
			$datas['ucuserid'] = $uid;
			...
			if ($s = $this->member_db->get_one(array('username'=>$r['username']))) {
				...
				$this->member_db->update($datas, array('userid'=>$s['userid']));
				//$data['uid'] = $s;
				$data['userid'] = $s['userid'];
			} else {
				//$data['uid'] = $this->member_db->insert($datas, true);
				$data['userid'] = $this->member_db->insert($datas, true);
			}
			//向所有的应用中发布新用户注册通知
			//bc_base::load_app_func('global', 'admin');
			//bc_base::load_app_class('messagequeue', 'admin' , 0);
	    ...
	    bc_base::load_sys_class('param', '', 0);
	    if(!$cookietime) $get_cookietime = param::get_cookie('cookietime');
	    $_cookietime = $cookietime ? intval($cookietime) : ($get_cookietime ? $get_cookietime : 0);
	    $cookietime = $_cookietime ? TIME + $_cookietime : 0;
	    
	    $bangcms_auth_key = md5(bc_base::load_config('system', 'auth_key').$_SERVER['HTTP_USER_AGENT']);
