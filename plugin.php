...
class plugin_base {
	...
	private function load_controller($filename = '', $m = '') {
		...
		$filepath = BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.$filename.'.class.php';
		if (file_exists($filepath) && $filename !='plugin_admin') {
			$classname = $filename;
			include $filepath;
			if ($mypath = bc_base::my_path($filepath)) {
	...
	final public static function plugin_tpl($file, $identification = '') {
		...
		return BC_PATH.'plugin'.DIRECTORY_SEPARATOR.$identification.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
	}
